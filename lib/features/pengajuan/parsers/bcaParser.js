...
function bcaParser(text) {
  const lines = text.split('\n');
  const transactions = [];

  // Pola umum BCA: "01/01/2024  KETERANGAN  1.000.000,00  500.000,00"
  ...
  for (const line of lines) {
    ...
    if (match) {
      ...
      transactions.push({
        ...
        // Untuk BCA, saldo biasanya tidak langsung ada, bisa dihitung nanti di frontend
        saldo: 0,
      });
    }
  }
