...
const slikResult = await pool.request()
  .input('id_pengajuan', sql.VarChar, id)
  .query(`
    SELECT 
      id_slik,
...
const listSlik = (slikResult.recordset || []).map((row) => {
  let parsed = {};
  try {
    parsed = row.slik_data ? JSON.parse(row.slik_data) : {};
  } catch (_) {}

  return {
    ...row,
    slik_data: parsed,
    jenis: parsed.jenis || parsed.tipe || 'DEBITUR',
    index: parsed.index ?? parsed.index_ke ?? null,
  };
});

const slikCheck = listSlik[0] || null;
