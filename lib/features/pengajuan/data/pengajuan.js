    ...
    try {
      ...
      SELECT
        ...
        CASE 
          ...
          WHEN p.stsflag = 9 THEN 'Putusan Kredit'
      ...
      WHERE ISNULL(p.stsflag, 0) IN (0, 1, 10, 2, 3, 4, 41, 5, 6, 7, 8, 9, 90, 91)
...
router.post('/api/pengajuan/:id/approval-muk', async (req, res) => {
    ...
    if (!targetStsflag) {
        return res.status(400).json({
            success: false,
            message: 'Aksi approval MUK tidak valid.',
        });
    }
    ...
    try {
        ...
        if (previousStsflag !== PENGAJUAN_STATUS.MUK_REVIEW) {
            return res.status(400).json({
                success: false,
                message: `Pengajuan tidak berada di tahap Approval MUK. Status saat ini ${previousStsflag || '-'}.`,
            });
        }
        ...
        if (action === 'approve' || action === 'setujui') {
            ...
            targetStsflag = afterMukReview(muk.recordset.length > 0);
        }
        ...
        res.json({
            ...
            message: targetStsflag === PENGAJUAN_STATUS.APPROVAL_FINAL
                ? 'Approval MUK disetujui. Pengajuan lanjut ke Approval Final.'
                : targetStsflag === PENGAJUAN_STATUS.MUK
                    ? 'Lanjutkan penyusunan atau perbaikan MUK.'
                    : 'Pengajuan ditolak.',
            previous_stsflag: previousStsflag,
            stsflag: targetStsflag,
        });
...
router.post('/api/pengajuan/:id/approval-direksi', async (req, res) => {
    ...
    const targetByAction = {
        approve: '100',
        setujui: '100',
        koreksi: PENGAJUAN_STATUS.MUK,
        tolak: PENGAJUAN_STATUS.DITOLAK,
        reject: PENGAJUAN_STATUS.DITOLAK,
    };
    ...
    if (!targetStsflag) {
        return res.status(400).json({
            success: false,
            message: 'Aksi approval direksi tidak valid.',
        });
    }
    ...
    try {
        ...
        if (previousStsflag !== PENGAJUAN_STATUS.APPROVAL_FINAL) {
            return res.status(409).json({
                success: false,
                message: `Pengajuan tidak berada di Approval Direksi. Status saat ini: ${previousStsflag}`,
            });
        }

        await pool.request()
            .input('id', sql.VarChar, id)
            .input('stsflag', sql.VarChar, targetStsflag)
            .query(`
                UPDATE t_pengajuan
                SET stsflag = @stsflag
                WHERE id_pengajuan = @id
        ...
        return res.json({
            ...
            message:
                ['approve', 'setujui'].includes(action)
                    ? 'Approval Direksi disetujui. Pengajuan selesai.'
                    : targetStsflag === PENGAJUAN_STATUS.MUK
                        ? 'Pengajuan dikoreksi kembali ke MUK.'
                        : 'Pengajuan ditolak.',
            previous_stsflag: previousStsflag,
            stsflag: targetStsflag,
        });
