...
const WORKFLOW_NEXT_STATUS = Object.freeze({
    VERIFIKASI_SIMPAN: PENGAJUAN_STATUS.APPROVAL_AWAL,
    ...
    CHECKLIST_LANJUT: PENGAJUAN_STATUS.REKAP_ANALISA,

    REKAP_ANALISA_SIMPAN: PENGAJUAN_STATUS.APPROVAL_REKAP,
    ...
    SURVEY_AGUNAN_SIMPAN: PENGAJUAN_STATUS.MUK_REVIEW,
    MUK_SETUJUI: PENGAJUAN_STATUS.APPROVAL_FINAL,

    APPROVAL_MUK_SETUJUI: PENGAJUAN_STATUS.MUK,
    APPROVAL_MUK_KOREKSI: PENGAJUAN_STATUS.SURVEY_AGUNAN,
...
router.post('/api/pengajuan/baru', handleUpload, async (req, res) => {
    ...
    const isFpk = finalStsflag === PENGAJUAN_STATUS.FPK_AO;
...
router.put('/api/pengajuan/:id', handleUpload, async (req, res) => {
    ...
    try {
        ...
        const shouldPreserveFpkFields = String(stsflag || '') !== PENGAJUAN_STATUS.FPK_AO;
...
router.post('/api/pengajuan/verifikasi/:id', handleUpload, async (req, res) => {
    ...
    try {
        ...
        // Update stsflag menjadi Approval Awal.
        await pool.request()
            .input('id', sql.VarChar, id)
            .input('stsflag', sql.VarChar, WORKFLOW_NEXT_STATUS.VERIFIKASI_SIMPAN)
            .query(`UPDATE t_pengajuan SET stsflag = @stsflag WHERE id_pengajuan = @id`);

        const whatsappNotification = queueWorkflowNotification(pool, {
            idPengajuan: id,
            targetStsflag: WORKFLOW_NEXT_STATUS.VERIFIKASI_SIMPAN,
            event: 'save',
        });
        ...
        res.json({
            ...
            stsflag: WORKFLOW_NEXT_STATUS.VERIFIKASI_SIMPAN,
...
router.post('/api/pengajuan/:id/rekap-analisa', async (req, res) => {
    try {
        ...
        await updatePengajuanStsflag(pool, idPengajuan, WORKFLOW_NEXT_STATUS.REKAP_ANALISA_SIMPAN);
        const whatsappNotification = queueWorkflowNotification(pool, {
            idPengajuan,
            targetStsflag: WORKFLOW_NEXT_STATUS.REKAP_ANALISA_SIMPAN,
            event: 'save',
        });
        res.status(200).json({
            status: 'success',
    ...
    try {
      ...
      SELECT
        ...
        CASE 
          ...
          WHEN p.stsflag = 7 THEN 'MUK / Review Manager'
          WHEN p.stsflag = 8 THEN 'Keputusan / Approval Final'
      ...
      WHERE ISNULL(p.stsflag, 0) IN (0, 1, 10, 2, 3, 4, 5, 6, 7, 8, 90, 91)
...
router.put('/api/users/:userid/theme-mode', async (req, res) => {
    try {
        ...
        const mode =
            String(theme_mode || '').toLowerCase() === 'dark'
                ? 'dark'
                : 'light';
    ...
    try {
        ...
        res.json({
            ...
            message: targetStsflag === PENGAJUAN_STATUS.APPROVAL_FINAL
                ...
                : targetStsflag === PENGAJUAN_STATUS.SURVEY_AGUNAN
                    ? 'Pengajuan dikoreksi ke Survey Agunan.'
                    : 'Pengajuan ditolak.',
            previous_stsflag: previousStsflag,
            stsflag: targetStsflag,
        });
