...
router.post('/api/pengajuan/baru', handleUpload, async (req, res) => {
    ...
    try {
        ...
        savedFiles = await saveUploadedFiles(id_pengajuan, relatedSignatureFiles(body, req.files));
        ...
        if (header.screening_awal) {
            ...
            const hasSlik = slikRows.recordset.some(row => {
                try {
                    ...
                    return data && ['DEBITUR', 'BADAN_USAHA'].includes(String(data.jenis || 'DEBITUR').toUpperCase()) && Boolean(data.nama_debitur || data.nama);
