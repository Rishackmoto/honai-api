    ...
    await pool.request().query(`
        ...
        BEGIN
            CREATE TABLE dbo.t_approval_policy (
                ...
                internal_employee_requires_komisaris BIT NOT NULL DEFAULT 1,
        ...
        BEGIN
            ...
            VALUES
                (1, ${APPROVAL_POLICY_DEFAULT_THRESHOLD}, 1, 1, GETDATE())
        END
        ...
        BEGIN
            CREATE TABLE dbo.t_approval_policy_history (
                ...
                komisaris_min_plafon DECIMAL(18,2) NOT NULL,
                internal_employee_requires_komisaris BIT NOT NULL,
...
function approvalRoutePayload({ idPengajuan, plafon, isInternal, policy, snapshot = false, snapshotAt = null }) {
    const threshold = Number(policy.komisaris_min_plafon || APPROVAL_POLICY_DEFAULT_THRESHOLD);
    const reasons = [];
    if (Number(plafon || 0) >= threshold) {
        reasons.push(`Plafon ${formatRupiahPolicy(plafon)} >= batas Komisaris ${formatRupiahPolicy(threshold)}`);
    }
    if (toBooleanFlag(policy.internal_employee_requires_komisaris) && isInternal) {
        reasons.push('Debitur merupakan Karyawan Internal BPR ANP');
    }
    const requiresKomisaris = reasons.length > 0;
    const route = ['Manager', 'Direksi', ...(requiresKomisaris ? ['Komisaris'] : [])];
    return {
        id_pengajuan: idPengajuan,
        plafon_pengajuan: Number(plafon || 0),
        is_karyawan_internal: !!isInternal,
        ...
        requires_komisaris: requiresKomisaris,
        alasan_route: reasons.join(' | ') || 'Plafon di bawah batas Komisaris dan bukan karyawan internal',
        route,
        jalur_approval: route.join(' → '),
        snapshot,
...
async function computeApprovalRoute(pool, idPengajuan) {
    ...
    const result = await pool.request()
        .input('id_pengajuan', sql.VarChar, idPengajuan)
        .query(`
            SELECT TOP 1 p.plafon_pengajuan,
    ...
    return approvalRoutePayload({
        idPengajuan,
        plafon: row.plafon_pengajuan,
        isInternal: toBooleanFlag(row.is_karyawan_internal),
        policy,
    });
}
...
async function getApprovalRoute(pool, idPengajuan) {
    ...
    const snapshotResult = await pool.request()
        ...
        .query(`
            SELECT TOP 1 policy_version, komisaris_min_plafon,
                   is_karyawan_internal, requires_komisaris,
                   alasan_route, route_json, snapshot_at
            FROM dbo.t_pengajuan_approval_route
            WHERE id_pengajuan = @id_pengajuan
        `);
    if (!snapshotResult.recordset.length) {
        return computeApprovalRoute(pool, idPengajuan);
    }
    ...
    if (!Array.isArray(route) || !route.length) {
        route = ['Manager', 'Direksi', ...(toBooleanFlag(row.requires_komisaris) ? ['Komisaris'] : [])];
    }
    return {
        id_pengajuan: idPengajuan,
        komisaris_min_plafon: Number(row.komisaris_min_plafon || 0),
        is_karyawan_internal: toBooleanFlag(row.is_karyawan_internal),
        requires_komisaris: toBooleanFlag(row.requires_komisaris),
...
async function saveApprovalRouteSnapshot(pool, idPengajuan, snapshotBy) {
    // Snapshot approval bersifat immutable. Jika sudah pernah dibentuk pada
    // Review MUK, perubahan policy berikutnya tidak boleh diam-diam mengubah
    // jalur approval pengajuan yang sedang berjalan.
    const existingSnapshot = await getApprovalRoute(pool, idPengajuan);
    if (existingSnapshot?.snapshot) return existingSnapshot;
            ...
            BEGIN
                INSERT INTO dbo.t_pengajuan_approval_route
                    (id_pengajuan, policy_version, komisaris_min_plafon, is_karyawan_internal,
                     requires_komisaris, alasan_route, route_json, snapshot_by, snapshot_at, updated_at)
                VALUES
                    (@id_pengajuan, @policy_version, @komisaris_min_plafon, @is_karyawan_internal,
                     @requires_komisaris, @alasan_route, @route_json, @snapshot_by, GETDATE(), GETDATE())
            END
        `);
...
// ================== END V10 MATRKS KEWENANGAN KREDIT ==================
...
router.post('/api/pengajuan/:id/muk', async (req, res) => {
    ...
    try {
        ...
        if (!isDraft) {
            await updatePengajuanStsflag(pool, id, WORKFLOW_NEXT_STATUS.MUK_SETUJUI);
            whatsappNotification = queueWorkflowNotification(pool, {
                idPengajuan: id,
                targetStsflag: WORKFLOW_NEXT_STATUS.MUK_SETUJUI,
                event: 'muk',
            });
        }
        ...
        res.json({
            status: 'success',
            message: isDraft ? 'Draft MUK berhasil disimpan' : 'MUK dikirim untuk review',
            is_draft: isDraft,
            whatsapp_notification: whatsappNotification,
        });
    } catch (error) {
    ...
    try {
        ...
        if (action === 'approve' || action === 'setujui') {
            ...
            if (!muk.recordset.length) {
                ...
                // Snapshot dibuat saat Manager menyetujui MUK. Kebijakan setelah titik ini
                // tidak mengubah jalur pengajuan yang sedang berjalan secara diam-diam.
                approvalRoute = await saveApprovalRouteSnapshot(pool, id, actor.userid);
                targetStsflag = PENGAJUAN_STATUS.APPROVAL_FINAL;
            }
        }
        ...
        return res.json({
            success: true,
            message: targetStsflag === PENGAJUAN_STATUS.APPROVAL_FINAL
                ? 'Review MUK disetujui Manager. Pengajuan lanjut ke Putusan Direksi.'
                : targetStsflag === PENGAJUAN_STATUS.MUK
...
router.post('/api/pengajuan/:id/approval-direksi', async (req, res) => {
    ...
    try {
        ...
        if (['approve', 'setujui'].includes(action)) {
            targetStsflag = approvalRoute.requires_komisaris
                ? PENGAJUAN_STATUS.APPROVAL_KOMISARIS
        ...
        const approvedMessage = approvalRoute.requires_komisaris
            ? `Putusan Direksi disetujui. Pengajuan wajib lanjut ke Persetujuan Komisaris (${approvalRoute.alasan_route}).`
            : 'Putusan Direksi disetujui. Pengajuan selesai dan dapat dilanjutkan ke Dokumen Kredit.';
...
router.post('/api/pengajuan/:id/approval-komisaris', async (req, res) => {
    ...
    try {
        ...
        if (!approvalRoute?.requires_komisaris) {
            return res.status(409).json({
                success: false,
                message: 'Snapshot jalur pengajuan ini tidak mensyaratkan Persetujuan Komisaris.',
            });
        }
