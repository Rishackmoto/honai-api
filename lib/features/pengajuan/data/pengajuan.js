...
// POST - Pengajuan Baru
const { screeningSubmissionError, screeningDecision } = require('./screening');
...
router.put('/api/pengajuan/:id', handleUpload, async (req, res) => {
    ...
    try {
        ...
        const screeningHeader = await new sql.Request(transaction)
            ...
            .query('SELECT p.stsflag, p.screening_awal, d.foto_ktp FROM t_pengajuan p WITH (UPDLOCK, HOLDLOCK) LEFT JOIN t_debitur_perorangan d ON p.id_pengajuan = d.id_pengajuan WHERE p.id_pengajuan = @id');
        ...
        const shouldPreserveEmptyPenjamin = jenis_debitur !== 'BADAN_USAHA' && String(stsflag || '') === PENGAJUAN_STATUS.FPK_AO && hasOwnPayload(body, 'list_penjamin') && Array.isArray(list_penjamin) && list_penjamin.length === 0;
