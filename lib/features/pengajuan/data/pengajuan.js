...
router.post('/api/pengajuan/slik/:id', async (req, res) => {
    ...
    try {
        ...
        WHERE id_pengajuan = @id_pengajuan
          AND UPPER(ISNULL(JSON_VALUE(CAST(slik_data AS NVARCHAR(MAX)), '$.jenis'), 'DEBITUR')) = @jenis
          AND ISNULL(JSON_VALUE(CAST(slik_data AS NVARCHAR(MAX)), '$.index'), '') = @index_key
...
router.get('/api/pengajuan/listApprovalAwal', async (req, res) => {
  try {
    ...
    const result = await pool.request()
      ...
      .query(`
        SELECT
          ...
          CASE 
            WHEN EXISTS (
              SELECT 1 FROM t_pengajuan_slik s
              WHERE s.id_pengajuan = p.id_pengajuan
            ) THEN 1 ELSE 0 
          END AS slik_valid,

          CASE 
            WHEN EXISTS (
              SELECT 1 FROM t_pengajuan_dukcapil dk
              WHERE dk.id_pengajuan = p.id_pengajuan
                AND ISNULL(dk.status_dukcapil, 0) = 1
            ) THEN 1 ELSE 0 
          END AS dukcapil_valid,

          CASE 
            WHEN EXISTS (
              SELECT 1 FROM t_pengajuan_dokumen doc
              WHERE doc.id_pengajuan = p.id_pengajuan
            ) THEN 1 ELSE 0 
          END AS dokumen_valid,

          (
            SELECT TOP 1 catatan_admin
            FROM t_pengajuan_dukcapil dk
            WHERE dk.id_pengajuan = p.id_pengajuan
            ORDER BY dk.created_at DESC
          ) AS catatan_admin

        FROM t_pengajuan p
        LEFT JOIN t_debitur_perorangan d
          ON p.id_pengajuan = CAST(d.id_pengajuan AS VARCHAR(50))
        LEFT JOIN t_debitur_badan_usaha b
          ON p.id_pengajuan = CAST(b.id_pengajuan AS VARCHAR(50))
        WHERE ISNULL(p.stsflag, '0') = '1'
          AND (
            @search = '%%'
            OR p.id_pengajuan LIKE @search
            OR d.nama_debitur LIKE @search
            OR b.nama_perusahaan LIKE @search
            OR p.id_ao LIKE @search
          )
    ...
    console.error('LIST APPROVAL AWAL ERROR:', error);
    res.status(500).json({
      success: false,
      message: 'Gagal memuat approval awal',
      error: error.message,
    });
  }
});
...
router.post('/api/pengajuan/:id/approval-awal', async (req, res) => {
  ...
  try {
    ...
    if (!['approve', 'koreksi', 'tolak'].includes(action)) {
      return res.status(400).json({
        success: false,
        message: 'Action approval tidak valid',
      });
    }

    if ((action === 'koreksi' || action === 'tolak') && !catatan) {
      return res.status(400).json({
        success: false,
        message: 'Catatan wajib diisi untuk koreksi / tolak',
      });
    }

    const pool = await getPool();
    await ensureApprovalAwalTable(pool);

    const summary = await getPengajuanNotificationSummary(pool, id, {
      id_pengajuan: id,
    });

    let nextStsflag = summary.stsflag;
    let nextStatus = summary.status_pengajuan;
    let message = '';

    if (action === 'approve') {
      nextStsflag = '2';
      nextStatus = 'DISETUJUI SUPERVISOR - LANJUT FPK';
      message = 'Pengajuan disetujui dan dilanjutkan ke FPK.';
    }

    if (action === 'koreksi') {
      nextStsflag = '1';
      nextStatus = 'KOREKSI VERIFIKASI';
      message = 'Pengajuan dikembalikan ke Admin Kredit untuk koreksi.';
    }

    if (action === 'tolak') {
      nextStsflag = '99';
      nextStatus = 'DITOLAK SUPERVISOR';
    ...
    try {
      await new sql.Request(transaction)
        .input('id_pengajuan', sql.VarChar, id)
        .input('stsflag', sql.VarChar, nextStsflag)
        .input('status_pengajuan', sql.VarChar, nextStatus)
        .query(`
          UPDATE t_pengajuan
          SET stsflag = @stsflag,
      ...
      await new sql.Request(transaction)
        ...
        .input('catatan', sql.NVarChar, catatan || null)
        .input('skor_kelengkapan', sql.Int, scoring.kelengkapan || null)
        .input('skor_slik', sql.Int, scoring.slik || null)
        .input('skor_dukcapil', sql.Int, scoring.dukcapil || null)
        .input('skor_risiko', sql.Int, scoring.risiko || null)
        .input('userid', sql.VarChar, userid || req.get('x-userid') || null)
        .input('username', sql.NVarChar, username || req.get('x-username') || null)
        .query(`
    ...
    const target = action === 'approve'
      ? workflowNotificationTargets['2']
      : action === 'koreksi'
        ? workflowNotificationTargets['1']
    ...
    notifyWorkflowUsersSafe(pool, {
      ...
      event: action === 'approve' ? 'save' : action === 'koreksi' ? 'koreksi' : 'delete',
      previousStsflag: summary.stsflag,
      catatan: catatan || null,
    }).catch((err) => {
      console.error(`[APPROVAL AWAL ${id}] WA gagal:`, err.message);
    });
  } catch (error) {
    console.error('APPROVAL AWAL ERROR:', error);
    res.status(500).json({
      success: false,
      message: 'Gagal memproses approval awal',
      error: error.message,
    });
  }
});
