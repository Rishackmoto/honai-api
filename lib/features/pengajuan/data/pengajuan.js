...
router.post('/api/pengajuan/verifikasi/:id', handleUpload, async (req, res) => {
    ...
    try {
        const pool = await getPool();
        await maybeEnsureSchema(ensurePengajuanDukcapilTable, pool);
        await maybeEnsureSchema(ensureVerifikasiDukcapilColumns, pool);
        ...
        const nextStatus = afterVerification(creditData.recordset.length > 0);
