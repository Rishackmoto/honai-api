...
const { sendWhatsAppTemplate } = require('../../../core/notification/whatsapp');
...
const runDbEnsureOnRequest = process.env.RUN_DB_ENSURE_ON_REQUEST === 'true';

const { PENGAJUAN_STATUS, WORKFLOW_NEXT_STATUS, isDraftSubmission, afterVerification, afterAnalisa, afterMukReview } = require('./workflow');
const dashboardNotificationRouter = require('./dashboard_notification');

const STATUS_LISTS = Object.freeze({
    VERIFIKASI: [PENGAJUAN_STATUS.VERIFIKASI_ADMIN, PENGAJUAN_STATUS.KOREKSI_ADMIN],
    APPROVAL_REKAP: [PENGAJUAN_STATUS.APPROVAL_REKAP],
    APPROVAL_AWAL: [PENGAJUAN_STATUS.APPROVAL_AWAL],
    FPK: [PENGAJUAN_STATUS.FPK_AO],
    CHECKLIST: [PENGAJUAN_STATUS.CHECKLIST_ADMIN],
    REKAP_ANALISA: [PENGAJUAN_STATUS.REKAP_ANALISA],
    SURVEY_DEBITUR: [PENGAJUAN_STATUS.SURVEY_DEBITUR],
    SURVEY_AGUNAN: [PENGAJUAN_STATUS.SURVEY_AGUNAN],
    MUK: [PENGAJUAN_STATUS.MUK],
    APPROVAL_DIREKSI: [PENGAJUAN_STATUS.APPROVAL_FINAL],
    APPROVAL_KOMISARIS: [PENGAJUAN_STATUS.APPROVAL_KOMISARIS],
});

const ALLOWED_PROGRESS_STATUSES = new Set([
    PENGAJUAN_STATUS.REKAP_ANALISA,
    PENGAJUAN_STATUS.APPROVAL_REKAP,
    PENGAJUAN_STATUS.SURVEY_DEBITUR,
    PENGAJUAN_STATUS.SURVEY_AGUNAN,
    PENGAJUAN_STATUS.MUK_REVIEW,
    PENGAJUAN_STATUS.MUK,
    PENGAJUAN_STATUS.APPROVAL_FINAL,
    PENGAJUAN_STATUS.APPROVAL_KOMISARIS,
]);

const ALLOWED_KOREKSI_STATUSES = new Set([
...
async function maybeEnsureSchema(ensureFn, pool) {
    if (!runDbEnsureOnRequest) return;
    await ensureFn(pool);
}

function createPerformanceTimer(res, operation, idPengajuan) {
    ...
    return {
        ...
        finish() {
            const now = process.hrtime.bigint();
            const totalMs = Number(now - started) / 1e6;
            const serverTiming = marks
                .map((item) => `${item.name};dur=${item.ms.toFixed(1)}`)
                .concat(`total;dur=${totalMs.toFixed(1)}`)
                .join(', ');
            try {
                res.setHeader('Server-Timing', serverTiming);
                res.setHeader('X-Honai-Save-Ms', totalMs.toFixed(0));
            } catch (_) {}
            console.log(`[PERF][${operation}] ${idPengajuan || '-'} total=${totalMs.toFixed(1)}ms ` +
                marks.map((item) => `${item.name}=${item.ms.toFixed(1)}ms`).join(' '));
            return totalMs;
        },
    };
}
...
// ==================== V10.2: RULE-BASED MATRKS KEWENANGAN KREDIT ====================
const APPROVAL_POLICY_DEFAULT_THRESHOLD = 100000000;
const {
    boolFlag: matrixBoolFlag,
    ...
    nextApprovalRole,
    statusForRole,
} = require('./approval_matrix');

function toBooleanFlag(value) {
    return matrixBoolFlag(value);
}

function formatRupiahPolicy(value) {
    const number = Number(value || 0);
    return `Rp ${Math.round(number).toLocaleString('id-ID')}`;
}

async function ensureApprovalMatrixTables(pool) {
    await pool.request().query(`
        IF OBJECT_ID('dbo.t_approval_policy', 'U') IS NULL
        BEGIN
            CREATE TABLE dbo.t_approval_policy (
                id_policy INT NOT NULL PRIMARY KEY,
                komisaris_min_plafon DECIMAL(18,2) NOT NULL,
                internal_employee_requires_komisaris BIT NOT NULL DEFAULT 0,
        ...
        IF NOT EXISTS (SELECT 1 FROM dbo.t_approval_policy WHERE id_policy = 1)
        BEGIN
            INSERT INTO dbo.t_approval_policy
                (id_policy, komisaris_min_plafon, internal_employee_requires_komisaris, version_no, updated_at)
            VALUES
                (1, ${APPROVAL_POLICY_DEFAULT_THRESHOLD}, 0, 1, GETDATE())
        END

        IF OBJECT_ID('dbo.t_approval_policy_rule', 'U') IS NULL
        BEGIN
            CREATE TABLE dbo.t_approval_policy_rule (
                id_rule INT IDENTITY(1,1) PRIMARY KEY,
                version_no INT NOT NULL DEFAULT 1,
                nama_rule NVARCHAR(200) NOT NULL,
                priority INT NOT NULL DEFAULT 100,
                kategori_debitur VARCHAR(40) NOT NULL DEFAULT 'SEMUA',
                sumber_pengembalian VARCHAR(40) NOT NULL DEFAULT 'SEMUA',
                plafon_mode VARCHAR(40) NOT NULL DEFAULT 'SEMUA',
                min_plafon DECIMAL(18,2) NULL,
                max_plafon DECIMAL(18,2) NULL,
                require_manager BIT NOT NULL DEFAULT 1,
                require_direksi BIT NOT NULL DEFAULT 1,
                require_komisaris BIT NOT NULL DEFAULT 0,
        ...
        IF NOT EXISTS (SELECT 1 FROM dbo.t_approval_policy_rule)
        BEGIN
            INSERT INTO dbo.t_approval_policy_rule
                (version_no, nama_rule, priority, kategori_debitur, sumber_pengembalian,
                 plafon_mode, require_manager, require_direksi, require_komisaris, aktif)
            VALUES
                (1, N'Plafon minimal batas kebijakan', 10, 'SEMUA', 'SEMUA', 'MINIMAL_BATAS', 1, 1, 1, 1),
                (1, N'Karyawan Internal - sumber gaji di bawah batas', 20, 'KARYAWAN_INTERNAL', 'GAJI', 'DI_BAWAH_BATAS', 1, 1, 0, 1),
                (1, N'Karyawan Internal - non gaji di bawah batas', 30, 'KARYAWAN_INTERNAL', 'SEMUA', 'DI_BAWAH_BATAS', 1, 1, 1, 1),
                (1, N'Debitur di bawah batas kebijakan', 40, 'SEMUA', 'SEMUA', 'DI_BAWAH_BATAS', 1, 1, 0, 1)
        END

        IF OBJECT_ID('dbo.t_approval_policy_history', 'U') IS NULL
        BEGIN
            CREATE TABLE dbo.t_approval_policy_history (
                id_history BIGINT IDENTITY(1,1) PRIMARY KEY,
                version_no INT NOT NULL,
                komisaris_min_plafon DECIMAL(18,2) NOT NULL,
                internal_employee_requires_komisaris BIT NOT NULL DEFAULT 0,
                rules_json NVARCHAR(MAX) NULL,
                alasan_perubahan NVARCHAR(500) NULL,
                changed_by VARCHAR(50) NULL,
                changed_by_name NVARCHAR(150) NULL,
                changed_at DATETIME NOT NULL DEFAULT GETDATE()
            )
        END
        IF COL_LENGTH('dbo.t_approval_policy_history', 'rules_json') IS NULL
            ALTER TABLE dbo.t_approval_policy_history ADD rules_json NVARCHAR(MAX) NULL

        IF OBJECT_ID('dbo.t_pengajuan_approval_context', 'U') IS NULL
        BEGIN
            CREATE TABLE dbo.t_pengajuan_approval_context (
                id_pengajuan VARCHAR(50) NOT NULL PRIMARY KEY,
                is_karyawan_internal BIT NOT NULL DEFAULT 0,
                nip_karyawan_internal VARCHAR(80) NULL,
                unit_kerja_internal NVARCHAR(150) NULL,
                jabatan_karyawan_internal NVARCHAR(150) NULL,
                updated_at DATETIME NOT NULL DEFAULT GETDATE()
            )
        END

        IF OBJECT_ID('dbo.t_pengajuan_approval_route', 'U') IS NULL
        BEGIN
            CREATE TABLE dbo.t_pengajuan_approval_route (
                id_pengajuan VARCHAR(50) NOT NULL PRIMARY KEY,
                policy_version INT NOT NULL,
                komisaris_min_plafon DECIMAL(18,2) NOT NULL,
                is_karyawan_internal BIT NOT NULL DEFAULT 0,
                sumber_pengembalian VARCHAR(40) NULL,
                matched_rule_id INT NULL,
                matched_rule_name NVARCHAR(200) NULL,
                requires_komisaris BIT NOT NULL DEFAULT 0,
                alasan_route NVARCHAR(1000) NULL,
                route_json NVARCHAR(MAX) NULL,
                route_revision INT NOT NULL DEFAULT 1,
                snapshot_by VARCHAR(50) NULL,
                snapshot_at DATETIME NOT NULL DEFAULT GETDATE(),
                updated_at DATETIME NOT NULL DEFAULT GETDATE()
            )
        END
        IF COL_LENGTH('dbo.t_pengajuan_approval_route', 'sumber_pengembalian') IS NULL
            ALTER TABLE dbo.t_pengajuan_approval_route ADD sumber_pengembalian VARCHAR(40) NULL
        IF COL_LENGTH('dbo.t_pengajuan_approval_route', 'matched_rule_id') IS NULL
            ALTER TABLE dbo.t_pengajuan_approval_route ADD matched_rule_id INT NULL
        IF COL_LENGTH('dbo.t_pengajuan_approval_route', 'matched_rule_name') IS NULL
            ALTER TABLE dbo.t_pengajuan_approval_route ADD matched_rule_name NVARCHAR(200) NULL
        IF COL_LENGTH('dbo.t_pengajuan_approval_route', 'route_revision') IS NULL
            ALTER TABLE dbo.t_pengajuan_approval_route ADD route_revision INT NOT NULL CONSTRAINT DF_t_pengajuan_approval_route_revision DEFAULT 1

        IF OBJECT_ID('dbo.t_pengajuan_approval_route_history', 'U') IS NULL
        BEGIN
            CREATE TABLE dbo.t_pengajuan_approval_route_history (
                id_history BIGINT IDENTITY(1,1) PRIMARY KEY,
        ...
        IF OBJECT_ID('dbo.t_pengajuan_approval_komisaris', 'U') IS NULL
        BEGIN
            CREATE TABLE dbo.t_pengajuan_approval_komisaris (
                id_approval INT IDENTITY(1,1) PRIMARY KEY,
                id_pengajuan VARCHAR(50) NOT NULL,
                action VARCHAR(20) NOT NULL,
                catatan NVARCHAR(MAX) NULL,
                previous_stsflag VARCHAR(10) NULL,
                target_stsflag VARCHAR(10) NULL,
        ...
        IF OBJECT_ID('dbo.t_debitur_data_kredit', 'U') IS NOT NULL
           AND COL_LENGTH('dbo.t_debitur_data_kredit', 'kategori_sumber_pengembalian') IS NULL
            ALTER TABLE dbo.t_debitur_data_kredit ADD kategori_sumber_pengembalian VARCHAR(40) NULL
    `);
}

async function saveApprovalContextTx(transaction, idPengajuan, approvalContext, jenisDebitur) {
    if (!approvalContext || typeof approvalContext !== 'object') return;

    const isInternal = jenisDebitur === 'PERORANGAN' && toBooleanFlag(approvalContext.is_karyawan_internal);
    const request = new sql.Request(transaction);
    await request
        .input('id_pengajuan', sql.VarChar, idPengajuan)
        .input('is_karyawan_internal', sql.Bit, isInternal)
        .input('nip_karyawan_internal', sql.VarChar, isInternal ? approvalContext.nip_karyawan_internal || null : null)
        .input('unit_kerja_internal', sql.NVarChar, isInternal ? approvalContext.unit_kerja_internal || null : null)
        .input('jabatan_karyawan_internal', sql.NVarChar, isInternal ? approvalContext.jabatan_karyawan_internal || null : null)
        .query(`
            IF EXISTS (SELECT 1 FROM dbo.t_pengajuan_approval_context WHERE id_pengajuan = @id_pengajuan)
            BEGIN
                UPDATE dbo.t_pengajuan_approval_context
                SET is_karyawan_internal = @is_karyawan_internal,
                    nip_karyawan_internal = @nip_karyawan_internal,
                    unit_kerja_internal = @unit_kerja_internal,
                    jabatan_karyawan_internal = @jabatan_karyawan_internal,
                    updated_at = GETDATE()
                WHERE id_pengajuan = @id_pengajuan
            END
            ELSE
            BEGIN
                INSERT INTO dbo.t_pengajuan_approval_context
                    (id_pengajuan, is_karyawan_internal, nip_karyawan_internal, unit_kerja_internal, jabatan_karyawan_internal, updated_at)
                VALUES
                    (@id_pengajuan, @is_karyawan_internal, @nip_karyawan_internal, @unit_kerja_internal, @jabatan_karyawan_internal, GETDATE())
            END
        `);
}

async function getWorkflowActor(pool, req) {
    const requestedUserid = req.body?.userid?.toString().trim() || req.get('x-userid')?.toString().trim() || '';
    if (!requestedUserid) return null;

    const result = await pool.request()
        .input('userid', sql.VarChar, requestedUserid)
        .query(`
            SELECT TOP 1 userid, username, jabat, levelid, flag
            FROM muser
            WHERE userid = @userid
              AND ISNULL(flag, '1') = '1'
        `);
...
async function requireWorkflowJabatan(pool, req, res, allowedJabatan, actionLabel) {
    const actor = await getWorkflowActor(pool, req);
    if (!actor) {
        res.status(401).json({
            success: false,
            message: `User aktif wajib teridentifikasi untuk ${actionLabel}.`,
        });
        return null;
    }

    const jabat = String(actor.jabat || '').trim();
    if (!allowedJabatan.map(String).includes(jabat)) {
        res.status(403).json({
            success: false,
            message: `${actionLabel} hanya dapat dilakukan oleh ${allowedJabatan.map(jabatanName).join(' / ')}. User saat ini: ${jabatanName(jabat)}.`,
        });
        return null;
    }
    return actor;
...
async function canManageApprovalPolicy(pool, req) {
    const actor = await getWorkflowActor(pool, req);
    if (!actor) return null;
    const jabat = String(actor.jabat || '').trim();
    const levelid = String(actor.levelid || '').trim();
    return (levelid === '5' || ['11', '17'].includes(jabat)) ? actor : null;
}

async function getApprovalPolicy(pool) {
    await ensureApprovalMatrixTables(pool);
    const result = await pool.request().query(`
        SELECT TOP 1 id_policy, komisaris_min_plafon,
               internal_employee_requires_komisaris,
               version_no, updated_by, updated_at
        FROM dbo.t_approval_policy
        WHERE id_policy = 1
    `);
    return result.recordset[0];
}

async function getApprovalRules(pool) {
    await ensureApprovalMatrixTables(pool);
    const result = await pool.request().query(`
        SELECT id_rule, version_no, nama_rule, priority, kategori_debitur,
               sumber_pengembalian, plafon_mode, min_plafon, max_plafon,
               require_manager, require_direksi, require_komisaris, aktif,
               created_at, updated_at
        FROM dbo.t_approval_policy_rule
        ORDER BY priority ASC, id_rule ASC
    `);
    return result.recordset || [];
}

function approvalStatusFromRoute(route, currentRole = null) {
    const role = currentRole ? nextApprovalRole(route, currentRole) : firstApprovalRole(route);
    return role ? statusForRole(role) : PENGAJUAN_STATUS.DISETUJUI;
}

function approvalRoleLabel(role) {
    return ({ MANAGER: 'Manager', DIREKSI: 'Direksi', KOMISARIS: 'Komisaris' })[role] || 'Selesai';
}

async function computeApprovalRoute(pool, idPengajuan) {
    const policy = await getApprovalPolicy(pool);
    const rules = await getApprovalRules(pool);
    const result = await pool.request()
        .input('id_pengajuan', sql.VarChar, idPengajuan)
        .query(`
            SELECT TOP 1 p.plafon_pengajuan, p.jenis_debitur,
                   ISNULL(c.is_karyawan_internal, 0) AS is_karyawan_internal,
                   dk.kategori_sumber_pengembalian,
                   dk.sumber_pembayaran_kredit
            FROM dbo.t_pengajuan p
            LEFT JOIN dbo.t_pengajuan_approval_context c
              ON c.id_pengajuan = p.id_pengajuan
            LEFT JOIN dbo.t_debitur_data_kredit dk
              ON CAST(dk.id_pengajuan AS VARCHAR(50)) = p.id_pengajuan
            WHERE p.id_pengajuan = @id_pengajuan
        `);
    if (!result.recordset.length) return null;

    const row = result.recordset[0];
    const threshold = Number(policy.komisaris_min_plafon || APPROVAL_POLICY_DEFAULT_THRESHOLD);
    const context = {
        plafon_pengajuan: Number(row.plafon_pengajuan || 0),
        is_karyawan_internal: toBooleanFlag(row.is_karyawan_internal),
        kategori_debitur: normalizeDebtorCategory(row.jenis_debitur, row.is_karyawan_internal),
        sumber_pengembalian: normalizeRepaymentCategory(
            row.kategori_sumber_pengembalian,
            row.sumber_pembayaran_kredit,
        ),
    };

    const matchedRule = selectApprovalRule(rules, context, threshold);
    let route = matchedRule ? routeFromRule(matchedRule) : ['MANAGER', 'DIREKSI'];
    if (!route.length) route = ['MANAGER'];

    const ruleName = matchedRule?.nama_rule || 'Fallback Manager → Direksi';
    const reasons = [
        `Aturan: ${ruleName}`,
        `Plafon ${formatRupiahPolicy(context.plafon_pengajuan)}`,
        `Kategori ${context.kategori_debitur}`,
        `Sumber ${context.sumber_pengembalian}`,
    ];

    return {
        id_pengajuan: idPengajuan,
        plafon_pengajuan: context.plafon_pengajuan,
        is_karyawan_internal: context.is_karyawan_internal,
        kategori_debitur: context.kategori_debitur,
        sumber_pengembalian: context.sumber_pengembalian,
        komisaris_min_plafon: threshold,
        policy_version: Number(policy.version_no || 1),
        matched_rule_id: matchedRule?.id_rule ?? null,
        matched_rule_name: ruleName,
        requires_manager: route.includes('MANAGER'),
        requires_direksi: route.includes('DIREKSI'),
        requires_komisaris: route.includes('KOMISARIS'),
        alasan_route: reasons.join(' | '),
        route,
        jalur_approval: routeLabel(route),
        snapshot: false,
        snapshot_at: null,
        route_revision: 0,
    };
...
async function getApprovalRoute(pool, idPengajuan) {
    await ensureApprovalMatrixTables(pool);
    const snapshotResult = await pool.request()
        .input('id_pengajuan', sql.VarChar, idPengajuan)
        .query(`
            SELECT TOP 1 policy_version, komisaris_min_plafon,
                   is_karyawan_internal, sumber_pengembalian,
                   matched_rule_id, matched_rule_name, requires_komisaris,
                   alasan_route, route_json, route_revision, snapshot_at
            FROM dbo.t_pengajuan_approval_route
            WHERE id_pengajuan = @id_pengajuan
        `);
    if (!snapshotResult.recordset.length) return computeApprovalRoute(pool, idPengajuan);

    const row = snapshotResult.recordset[0];
    let route;
    try { route = JSON.parse(row.route_json || '[]'); } catch (_) { route = []; }
    route = normalizeRoute(route);
    if (!route.length) route = ['MANAGER', 'DIREKSI', ...(toBooleanFlag(row.requires_komisaris) ? ['KOMISARIS'] : [])];

    return {
        id_pengajuan: idPengajuan,
        komisaris_min_plafon: Number(row.komisaris_min_plafon || 0),
        is_karyawan_internal: toBooleanFlag(row.is_karyawan_internal),
        sumber_pengembalian: row.sumber_pengembalian || 'LAINNYA',
        matched_rule_id: row.matched_rule_id ?? null,
        matched_rule_name: row.matched_rule_name || '',
        requires_manager: route.includes('MANAGER'),
        requires_direksi: route.includes('DIREKSI'),
        requires_komisaris: route.includes('KOMISARIS'),
        alasan_route: row.alasan_route || '',
...
async function saveApprovalRouteSnapshot(pool, idPengajuan, snapshotBy, { forceRefresh = false } = {}) {
    const current = await getApprovalRoute(pool, idPengajuan);
    if (current?.snapshot && !forceRefresh) return current;

    const route = await computeApprovalRoute(pool, idPengajuan);
    if (!route) throw new Error('Data pengajuan tidak ditemukan untuk penentuan jalur approval.');

    const existingResult = await pool.request()
        .input('id_pengajuan', sql.VarChar, idPengajuan)
        .query(`SELECT TOP 1 route_revision FROM dbo.t_pengajuan_approval_route WHERE id_pengajuan = @id_pengajuan`);
    const nextRevision = (Number(existingResult.recordset[0]?.route_revision || 0) + 1);

    const request = pool.request()
        .input('id_pengajuan', sql.VarChar, idPengajuan)
        .input('policy_version', sql.Int, route.policy_version)
        .input('komisaris_min_plafon', sql.Decimal(18, 2), route.komisaris_min_plafon)
        .input('is_karyawan_internal', sql.Bit, route.is_karyawan_internal)
        .input('sumber_pengembalian', sql.VarChar, route.sumber_pengembalian)
        .input('matched_rule_id', sql.Int, route.matched_rule_id)
        .input('matched_rule_name', sql.NVarChar, route.matched_rule_name)
        .input('requires_komisaris', sql.Bit, route.requires_komisaris)
        .input('alasan_route', sql.NVarChar, route.alasan_route)
        .input('route_json', sql.NVarChar, JSON.stringify(route.route))
        .input('route_revision', sql.Int, nextRevision)
        .input('snapshot_by', sql.VarChar, snapshotBy || null);

    await request.query(`
        IF EXISTS (SELECT 1 FROM dbo.t_pengajuan_approval_route WHERE id_pengajuan = @id_pengajuan)
        BEGIN
            UPDATE dbo.t_pengajuan_approval_route
            SET policy_version = @policy_version,
                komisaris_min_plafon = @komisaris_min_plafon,
                is_karyawan_internal = @is_karyawan_internal,
                sumber_pengembalian = @sumber_pengembalian,
                matched_rule_id = @matched_rule_id,
                matched_rule_name = @matched_rule_name,
                requires_komisaris = @requires_komisaris,
                alasan_route = @alasan_route,
        ...
        ELSE
        BEGIN
            INSERT INTO dbo.t_pengajuan_approval_route
                (id_pengajuan, policy_version, komisaris_min_plafon, is_karyawan_internal,
                 sumber_pengembalian, matched_rule_id, matched_rule_name, requires_komisaris,
                 alasan_route, route_json, route_revision, snapshot_by, snapshot_at, updated_at)
            VALUES
                (@id_pengajuan, @policy_version, @komisaris_min_plafon, @is_karyawan_internal,
                 @sumber_pengembalian, @matched_rule_id, @matched_rule_name, @requires_komisaris,
                 @alasan_route, @route_json, @route_revision, @snapshot_by, GETDATE(), GETDATE())
        END

        INSERT INTO dbo.t_pengajuan_approval_route_history
            (id_pengajuan, route_revision, policy_version, komisaris_min_plafon,
             is_karyawan_internal, sumber_pengembalian, matched_rule_id, matched_rule_name,
             alasan_route, route_json, snapshot_by, snapshot_at)
        VALUES
            (@id_pengajuan, @route_revision, @policy_version, @komisaris_min_plafon,
             @is_karyawan_internal, @sumber_pengembalian, @matched_rule_id, @matched_rule_name,
             @alasan_route, @route_json, @snapshot_by, GETDATE())
    `);

    return { ...route, snapshot: true, route_revision: nextRevision, snapshot_at: new Date().toISOString() };
}

async function enrichRowsWithApprovalRoute(pool, rows = []) {
    const enriched = [];
    for (const row of rows) {
        const route = await getApprovalRoute(pool, row.id_pengajuan);
        enriched.push({
            ...row,
            requires_manager: route?.requires_manager ?? true,
            requires_direksi: route?.requires_direksi ?? true,
            requires_komisaris: route?.requires_komisaris ?? false,
            is_karyawan_internal: route?.is_karyawan_internal ?? false,
            sumber_pengembalian: route?.sumber_pengembalian || 'LAINNYA',
            matched_rule_name: route?.matched_rule_name || '',
            approval_reason: route?.alasan_route || '',
            jalur_approval: route?.jalur_approval || 'Manager → Direksi',
            policy_version: route?.policy_version || null,
            route_revision: route?.route_revision || null,
        });
    }
    return enriched;
}
// ================== END V10.2 RULE-BASED MATRKS KEWENANGAN KREDIT ==================
...
async function ensureMuserThemeModeColumn(pool) {
    await pool.request().query(`
    IF COL_LENGTH('dbo.muser', 'theme_mode') IS NULL
    BEGIN
      ALTER TABLE dbo.muser ADD theme_mode VARCHAR(20) NULL
    END
  `);
}

router.use(auditLogger);
router.use(dashboardNotificationRouter);
...
async function getAccessTables(pool) {
    ...
    if (!menuTable || !roleTable) return null;
    return { menuTable, roleTable };
}

async function getUserMenuAccess(pool, user) {
    const levelAkses = Number(user.levelid || 0);
    const jabatan = user.jabat ? Number(user.jabat) : null;
    const accessTables = await getAccessTables(pool);

    if (!levelAkses || !accessTables) return [];

    const { menuTable, roleTable } = accessTables;
        ...
        SELECT
          *,
          ROW_NUMBER() OVER (
            ...
            ORDER BY
              CASE WHEN @jabatan IS NOT NULL AND jabatan = @jabatan THEN 0 ELSE 1 END,
              CASE WHEN level_akses = @level_akses THEN 0 ELSE 1 END
          ) AS rn
        FROM selected_role
      )
      SELECT
...
router.post('/api/login', async (req, res) => {
    const timer = `LOGIN-${Date.now()}-${Math.random()}`;
    console.time(timer);
    ...
    try {
        ...
        await maybeEnsureSchema(ensureMuserThemeModeColumn, pool);

        const result = await pool.request()
            .input('userid', sql.VarChar(30), userid)
            .query(`
                SELECT TOP 1
                    noref, userid, username, levelid, flag, email, nohp,
                    jabat, kdcab, foto_profile, theme_mode, pass
                FROM muser
                WHERE userid = @userid
                  AND ISNULL(flag, '1') = '1'
            `);
        ...
        const access = await getUserMenuAccess(pool, user);
...
router.get('/api/users/:userid/access', async (req, res) => {
    try {
        ...
        const result = await pool.request()
            ...
            .query(`
                SELECT TOP 1
                    noref, userid, username, levelid, flag, email, nohp,
                    jabat, kdcab, foto_profile, theme_mode
                FROM muser
                WHERE userid = @userid
                  AND ISNULL(flag, '1') = '1'
            `);
        ...
        return res.status(500).json({
            success: false,
            message: 'Gagal memuat hak akses user',
        });
    }
});
...
router.get('/api/users', async (req, res) => {
    try {
        const pool = await getPool();
        await maybeEnsureSchema(ensureMuserPasswordColumn, pool);
        await maybeEnsureSchema(ensureMuserProfileColumn, pool);
        const result = await pool.request().query(`
...
async function getMukData(pool, idPengajuan) {
    await maybeEnsureSchema(ensureMukTable, pool);
    const result = await pool.request()
        .input('id_pengajuan', sql.VarChar, idPengajuan)
        .query(`
...
async function insertPengajuanLog(pool, data) {
    await maybeEnsureSchema(ensurePengajuanLogTable, pool);
    await pool.request()
...
async function ensurePengajuanDetailTables(pool) {
    await ensureTableColumns(pool, 't_pengajuan', [
        { name: 'screening_awal', type: 'BIT' },
        { name: 'persetujuan_slik', type: 'NVARCHAR(MAX)' },
    ]);
    ...
    await ensureTableColumns(pool, 't_pengajuan_penjamin', [
        { name: 'agama', type: 'VARCHAR(50)' },
        { name: 'nama_ibu_kandung', type: 'VARCHAR(150)' },
        { name: 'pekerjaan', type: 'VARCHAR(150)' },
        { name: 'alamat_ktp', type: 'TEXT' },
        { name: 'alamat_domisili', type: 'TEXT' },
        { name: 'domisili_sesuai_ktp', type: 'BIT' },
    ...
    await ensureTableColumns(pool, 't_debitur_data_kredit', [
        ...
        { name: 'kategori_sumber_pengembalian', type: 'VARCHAR(40)' },
...
function parseKtpOcrText(text) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (!result.alamat && upperLine.includes('ALAMAT')) {
            const inlineAddress = valueAfterLabel(line, ['ALAMAT']);
            const alamat = inlineAddress ? [inlineAddress] : [];
            let j = i + 1;
            while (j < lines.length && !/^(?:RT|RW|KEL|DESA|KECAMATAN|AGAMA|STATUS|PEKERJAAN|KEWARGANEGARAAN|BERLAKU)/i.test(lines[j]) && lines[j].trim()) {
        ...
        const rtRwMatch = line.match(/RT\s*\/\s*RW\s*:?\s*(\d+)\s*\/\s*(\d+)/i) || line.match(/RT\s*(\d+)\s*\/\s*RW\s*(\d+)/i);
...
router.post('/api/ktp/scan', handleUpload, async (req, res) => {
    const image =
        req.files?.find((file) => file.fieldname === 'ktp_image') || req.files?.[0];
    ...
    try {
        ...
        const { data: ocrData } = await worker.recognize(image.buffer);

        const parsed = parseKtpOcrText(ocrData.text);
...
async function insertDataKredit(transaction, idPengajuan, dataKredit) {
    ...
    await new sql.Request(transaction)
        ...
        .input('kategori_sumber_pengembalian', sql.VarChar, normalizeRepaymentCategory(dataKredit.kategori_sumber_pengembalian, dataKredit.sumber_pembayaran_kredit))
        ...
        .query(`INSERT INTO t_debitur_data_kredit (id_pengajuan, referensi, nama_referensi, hubungan_dengan_bank, tujuan_penggunaan, jumlah_pengajuan_kredit, jangka_waktu_bulan, bunga_per_tahun, jenis_hitung_bunga, jenis_kredit, no_rekening_bank_lain, sumber_pembayaran_kredit, kategori_sumber_pengembalian, detail_tujuan, sindikasi, alasan, asuransi_jiwa, asuransi_kredit, asuransi_lainnya) VALUES (@id_pengajuan, @referensi, @nama_referensi, @hubungan_dengan_bank, @tujuan_penggunaan, @jumlah_pengajuan_kredit, @jangka_waktu_bulan, @bunga_per_tahun, @jenis_hitung_bunga, @jenis_kredit, @no_rekening_bank_lain, @sumber_pembayaran_kredit, @kategori_sumber_pengembalian, @detail_tujuan, @sindikasi, @alasan, @asuransi_jiwa, @asuransi_kredit, @asuransi_lainnya)`);
}
...
// POST - Pengajuan Baru
const { screeningSubmissionError, screeningDecision, readConsent, screeningIdentityChanged } = require('./screening');

// Reuse document storage for drawn signatures; signature columns hold file paths.
function relatedSignatureFiles(body, files = []) {
    const result = [...files];
    for (const role of ['pendiri', 'penjamin']) {
        for (const [index, person] of (body[`list_${role}`] || []).entries()) {
            const value = String(person[`ttd_${role}`] || '');
            const encoded = value.replace(/^data:image\/png;base64,/, '');
            if (!encoded.startsWith('iVBORw0KGgo') || !/^[A-Za-z0-9+/=\r\n]+$/.test(encoded)) continue;
            const buffer = Buffer.from(encoded, 'base64');
            if (buffer.length > 2 * 1024 * 1024) throw new Error('Ukuran tanda tangan terlalu besar.');
            const fieldname = `ttd_${role}_${index}`;
            if (!result.some(f => f.fieldname === fieldname)) result.push({
                fieldname, originalname: `${fieldname}.png`, mimetype: 'image/png', buffer, size: buffer.length,
            });
        }
    }
    return result;
...
router.post('/api/pengajuan/baru', handleUpload, async (req, res) => {
    ...
    const screeningError = screeningSubmissionError(body, req.files);
    if (screeningError) return res.status(400).json({ status: 'error', message: screeningError });
    const finalStsflag = stsflag || PENGAJUAN_STATUS.VERIFIKASI_ADMIN;
    const isFpk = body.form_lengkap === true || finalStsflag === PENGAJUAN_STATUS.FPK_AO;

    let transaction;
    let transactionStarted = false;
    let savedFiles = [];
    perf = createPerformanceTimer(res, 'SAVE_CREATE', id_pengajuan);
    ...
    try {
        ...
        const filesPromise = saveUploadedFiles(id_pengajuan, relatedSignatureFiles(body, req.files));
        const poolPromise = getPool();
        const [pool, uploadedFiles] = await Promise.all([poolPromise, filesPromise]);
        ...
        await maybeEnsureSchema(ensureDokumenTable, pool);
        await maybeEnsureSchema(ensurePengajuanDetailTables, pool);
        await maybeEnsureSchema(ensureApprovalMatrixTables, pool);
        perf.mark('schema');
        ...
        await saveApprovalContextTx(transaction, id_pengajuan, body.approval_context, jenis_debitur);

        if (body.screening_awal === true) {
            await new sql.Request(transaction)
                .input('id', sql.VarChar, id_pengajuan)
                .input('consent', sql.NVarChar(sql.MAX), JSON.stringify({ ...body.persetujuan_slik, recorded_at: new Date().toISOString(), recorded_by: id_ao }))
                .query('UPDATE t_pengajuan SET screening_awal = 1, persetujuan_slik = @consent WHERE id_pengajuan = @id');
        }
        ...
        if (jenis_debitur === 'PERORANGAN' && data_perorangan) {
            ...
            if (isFpk || body.screening_awal === true) {
        ...
        // Penjamin & Pendiri/Pengurus dikirim juga dari pengajuan awal.
        {
            if (list_penjamin && list_penjamin.length > 0) {
                for (let index = 0; index < list_penjamin.length; index++) {
                    ...
                    await new sql.Request(transaction)
                        ...
                        .input('ttd_penjamin', sql.NVarChar, getFilePath(savedFiles, `ttd_penjamin_${index}`) || penjamin.ttd_penjamin || null)
                        ...
                        .input('agama', sql.VarChar(50), penjamin.agama || null)
                        .input('nama_ibu_kandung', sql.VarChar(150), penjamin.nama_ibu_kandung || null)
                        .input('pekerjaan', sql.VarChar(150), penjamin.pekerjaan || null)
                        .input('alamat_ktp', sql.Text, penjamin.alamat_ktp || null)
                        .input('alamat_domisili', sql.Text, penjamin.domisili_sesuai_ktp ? penjamin.alamat_ktp || null : penjamin.alamat_domisili || null)
                        .input('domisili_sesuai_ktp', sql.Bit, penjamin.domisili_sesuai_ktp || false)
                        .query(`INSERT INTO t_pengajuan_penjamin 
                            (id_pengajuan, nama_penjamin, ktp_penjamin, hubungan_debitur, no_hp_penjamin, foto_penjamin, ttd_penjamin, tempat_lahir, tanggal_lahir, jenis_kelamin, tgl_berlaku_ktp, ktp_berlaku_seumur_hidup, agama, nama_ibu_kandung, pekerjaan, alamat_ktp, alamat_domisili, domisili_sesuai_ktp)
                            VALUES (@id_pengajuan, @nama_penjamin, @ktp_penjamin, @hubungan_debitur, @no_hp_penjamin, @foto_penjamin, @ttd_penjamin, @tempat_lahir, @tanggal_lahir, @jenis_kelamin, @tgl_berlaku_ktp, @ktp_berlaku_seumur_hidup, @agama, @nama_ibu_kandung, @pekerjaan, @alamat_ktp, @alamat_domisili, @domisili_sesuai_ktp)`);
                }
            }
            if (list_pendiri && list_pendiri.length > 0) {
                for (let index = 0; index < list_pendiri.length; index++) {
                    ...
                    await new sql.Request(transaction)
                        ...
                        .input('ttd_pendiri', sql.NVarChar, getFilePath(savedFiles, `ttd_pendiri_${index}`) || pendiri.ttd_pendiri || null)
        ...
        const whatsappNotification = queueWorkflowNotification(pool, {
...
router.put('/api/pengajuan/:id', handleUpload, async (req, res) => {
    ...
    let transactionStarted = false;
    let savedFiles = [];
    perf = createPerformanceTimer(res, 'SAVE_UPDATE', id_pengajuan);
    ...
    try {
        ...
        const screeningHeader = await new sql.Request(transaction)
            .input('id', sql.VarChar, req.params.id)
            .query('SELECT p.stsflag, p.screening_awal, p.persetujuan_slik, d.foto_ktp, d.foto_pasangan FROM t_pengajuan p WITH (UPDLOCK, HOLDLOCK) LEFT JOIN t_debitur_perorangan d ON p.id_pengajuan = d.id_pengajuan WHERE p.id_pengajuan = @id');
        const existingHeader = screeningHeader.recordset[0];
        if (!existingHeader || String(id_pengajuan) !== String(req.params.id)) {
            await transaction.rollback(); transactionStarted = false;
            await cleanupSavedFiles(savedFiles);
            return res.status(404).json({ status: 'error', message: 'Pengajuan tidak ditemukan atau ID tidak sesuai.' });
        }
        const screeningError = screeningSubmissionError(body, req.files, existingHeader);
        if (screeningError) {
            await transaction.rollback(); transactionStarted = false;
            await cleanupSavedFiles(savedFiles);
            return res.status(400).json({ status: 'error', message: screeningError });
        }
        if ((body.screening_awal === true || existingHeader.screening_awal) && ['0', '90'].includes(String(existingHeader.stsflag))) {
            await new sql.Request(transaction)
                .input('id', sql.VarChar, id_pengajuan)
                .input('consent', sql.NVarChar(sql.MAX), JSON.stringify({ ...body.persetujuan_slik, recorded_at: new Date().toISOString(), recorded_by: id_ao }))
                .query('UPDATE t_pengajuan SET screening_awal = 1, persetujuan_slik = @consent WHERE id_pengajuan = @id');
        }
        const shouldPreserveFpkFields = body.form_lengkap !== true && String(stsflag || '') !== PENGAJUAN_STATUS.FPK_AO;
        ...
        if (hasOwnPayload(body, 'approval_context')) {
            await saveApprovalContextTx(transaction, id_pengajuan, body.approval_context, jenis_debitur);
        }
        ...
        const oldPenjamin = await new sql.Request(transaction)
            .input('id', sql.VarChar, id_pengajuan)
            .query(`SELECT * FROM t_pengajuan_penjamin WHERE id_pengajuan = @id ORDER BY id_penjamin`);
        const oldPendiri = await new sql.Request(transaction)
            .input('id', sql.VarChar, id_pengajuan)
            .query(`SELECT * FROM t_pengajuan_pendiri WHERE id_pengajuan = @id ORDER BY id_pendiri`);
        if (existingHeader.screening_awal && String(existingHeader.stsflag) === '2' && screeningIdentityChanged(body, {
            perorangan: { ...oldPerorangan, ...oldDetailDebitur, ...oldDetailPasangan,
                ttd_debitur: oldPerorangan.effective_ttd_debitur, ttd_pasangan: oldPerorangan.effective_ttd_pasangan },
            badanUsaha: oldBU, pendiri: oldPendiri.recordset, penjamin: oldPenjamin.recordset,
        }, req.files)) {
            await transaction.rollback(); transactionStarted = false;
            await cleanupSavedFiles(savedFiles);
            return res.status(409).json({ status: 'error', message: 'Identitas yang telah diperiksa berubah. Gunakan Koreksi ke Pengajuan Kredit agar admin memeriksa kembali.' });
        }
        const shouldPreserveEmptyPenjamin = !existingHeader.screening_awal && jenis_debitur !== 'BADAN_USAHA' && String(stsflag || '') === PENGAJUAN_STATUS.FPK_AO && hasOwnPayload(body, 'list_penjamin') && Array.isArray(list_penjamin) && list_penjamin.length === 0;
        const shouldPreserveEmptyPendiri = String(stsflag || '') === PENGAJUAN_STATUS.FPK_AO && hasOwnPayload(body, 'list_pendiri') && Array.isArray(list_pendiri) && list_pendiri.length === 0;
        ...
        for (let index = 0; index < effectivePenjamin.length; index++) {
            ...
            const oldPenjaminFile = jenis_debitur === 'BADAN_USAHA'
                ? oldPenjamin.recordset.find(x => penjamin.id_penjamin != null && String(x.id_penjamin) === String(penjamin.id_penjamin))
                    || oldPenjamin.recordset.find(x => x.ktp_penjamin === penjamin.ktp_penjamin && x.nama_penjamin === penjamin.nama_penjamin)
                : oldPenjamin.recordset.find(x => x.nama_penjamin === penjamin.nama_penjamin);
            const mergedPenjamin = jenis_debitur === 'BADAN_USAHA'
                ? { ...(oldPenjaminFile || {}), ...penjamin }
                : mergePreservedFields(oldPenjaminFile || {}, penjamin, shouldPreserveFpkFields);
            const fotoPenjamin = getFilePath(savedFiles, `file_penjamin_${index}`) || (
                jenis_debitur === 'BADAN_USAHA' && hasOwnPayload(penjamin, 'foto_penjamin_url')
                    ? penjamin.foto_penjamin_url || null
                    : oldPenjaminFile?.foto_penjamin || mergedPenjamin.foto_penjamin_url || mergedPenjamin.foto_penjamin);
            ...
            await new sql.Request(transaction)
                ...
                .input('agama', sql.VarChar(50), mergedPenjamin.agama || null)
                .input('nama_ibu_kandung', sql.VarChar(150), mergedPenjamin.nama_ibu_kandung || null)
                .input('pekerjaan', sql.VarChar(150), mergedPenjamin.pekerjaan || null)
                .input('alamat_ktp', sql.Text, mergedPenjamin.alamat_ktp || null)
                .input('alamat_domisili', sql.Text, mergedPenjamin.domisili_sesuai_ktp ? mergedPenjamin.alamat_ktp || null : mergedPenjamin.alamat_domisili || null)
                .input('domisili_sesuai_ktp', sql.Bit, mergedPenjamin.domisili_sesuai_ktp || false)
                .query(`INSERT INTO t_pengajuan_penjamin (id_pengajuan, nama_penjamin, ktp_penjamin, hubungan_debitur, no_hp_penjamin, foto_penjamin,ttd_penjamin,tempat_lahir, tanggal_lahir, jenis_kelamin, tgl_berlaku_ktp, ktp_berlaku_seumur_hidup, agama, nama_ibu_kandung, pekerjaan, alamat_ktp, alamat_domisili, domisili_sesuai_ktp)
                     VALUES (@id_pengajuan, @nama_penjamin, @ktp_penjamin, @hubungan_debitur, @no_hp_penjamin, @foto_penjamin,@ttd_penjamin,@tempat_lahir, @tanggal_lahir, @jenis_kelamin, @tgl_berlaku_ktp, @ktp_berlaku_seumur_hidup, @agama, @nama_ibu_kandung, @pekerjaan, @alamat_ktp, @alamat_domisili, @domisili_sesuai_ktp)`);
        }
        ...
        const whatsappNotification = stsflag
            ? queueWorkflowNotification(pool, {
                idPengajuan: id_pengajuan,
                fallback: body,
...
function sqlInParams(request, prefix, values) {
    return values.map((value, index) => {
        const name = `${prefix}${index}`;
        request.input(name, sql.VarChar, String(value));
        return `@${name}`;
    }).join(', ');
}

// GET - List Verifikasi (stsflag = 1, 91)
router.get('/api/pengajuan/listVerifikasi', async (req, res) => {
    try {
        const pool = await getPool();
        const request = pool.request();
        const statusParams = sqlInParams(request, 'stsflag', STATUS_LISTS.VERIFIKASI);
        const result = await request.query(`
            ...
            WHERE a.stsflag IN (${statusParams})
...
router.get('/api/pengajuan/listApprovalAwal', async (req, res) => {
    try {
        const pool = await getPool();
        const search = req.query.search?.toString().trim() || '';
        const request = pool.request()
            .input('search', sql.NVarChar, `%${search}%`);
        const statusParams = sqlInParams(request, 'stsflag', STATUS_LISTS.APPROVAL_AWAL);
        const result = await request.query(`
            SELECT a.id_pengajuan, a.jenis_debitur, a.plafon_pengajuan, a.tenor_bulan,
                   a.tgl_pengajuan, a.status_pengajuan, a.catatan_ao, a.stsflag,
                   a.id_ao,
                   u.username AS ao_username,
                   COALESCE(u.username, a.id_ao, '-') AS ao,
                   b.nama_debitur, b.no_ktp, b.no_hp,
                   c.nama_perusahaan, c.npwp,
                   v.catatan_admin,
                   CASE WHEN a.jenis_debitur = 'PERORANGAN' THEN b.nama_debitur ELSE c.nama_perusahaan END AS nama
            FROM t_pengajuan a
            LEFT JOIN t_debitur_perorangan b ON a.id_pengajuan = CAST(b.id_pengajuan AS VARCHAR(50))
            LEFT JOIN t_debitur_badan_usaha c ON a.id_pengajuan = CAST(c.id_pengajuan AS VARCHAR(50))
            LEFT JOIN muser u ON LTRIM(RTRIM(CAST(u.userid AS VARCHAR(50)))) = LTRIM(RTRIM(CAST(a.id_ao AS VARCHAR(50))))
            OUTER APPLY (
                SELECT TOP 1 vd.catatan_admin
                FROM t_verifikasi_dukcapil vd
                WHERE vd.id_pengajuan = a.id_pengajuan
                ORDER BY vd.verified_at DESC
            ) v
            WHERE a.stsflag IN (${statusParams})
              AND (
                    @search = '%%'
                    OR a.id_pengajuan LIKE @search
                    OR ISNULL(a.id_ao, '') LIKE @search
                    OR ISNULL(u.username, '') LIKE @search
                    OR ISNULL(b.nama_debitur, '') LIKE @search
                    OR ISNULL(b.no_ktp, '') LIKE @search
                    OR ISNULL(c.nama_perusahaan, '') LIKE @search
                    OR ISNULL(c.npwp, '') LIKE @search
              )
            ORDER BY a.tgl_pengajuan ASC
        `);
        res.json({ status: 'success', data: result.recordset });
    } catch (error) {
        console.error('Error di listApprovalAwal:', error);
        res.status(500).json({ status: 'error', message: error.message });
    }
});

router.post('/api/pengajuan/:id/approval-awal', async (req, res) => {
    const { id } = req.params;
    const action = (req.body?.action || '').toString().toLowerCase().trim();
    const catatan = req.body?.catatan?.toString().trim() || null;
    const userid = req.body?.userid?.toString().trim() || req.get('x-userid') || null;
    const username = req.body?.username?.toString().trim() || req.get('x-username') || null;

    const targetByAction = {
        approve: WORKFLOW_NEXT_STATUS.APPROVAL_AWAL_SETUJUI,
        setujui: WORKFLOW_NEXT_STATUS.APPROVAL_AWAL_SETUJUI,
        koreksi: WORKFLOW_NEXT_STATUS.APPROVAL_AWAL_KOREKSI,
        tolak: WORKFLOW_NEXT_STATUS.APPROVAL_AWAL_TOLAK,
        reject: WORKFLOW_NEXT_STATUS.APPROVAL_AWAL_TOLAK,
    };
    const targetStsflag = targetByAction[action];

    if (!targetStsflag) {
        return res.status(400).json({
            status: 'error',
            success: false,
            message: 'Aksi approval awal tidak valid.',
        });
    }

    if ((action === 'koreksi' || action === 'tolak' || action === 'reject') && !catatan) {
        return res.status(400).json({
            status: 'error',
            success: false,
    ...
    try {
        ...
        const actor = await requireWorkflowJabatan(realPool, req, res, ['14'], 'Approval Awal');
        if (!actor) return;
        await ensureApprovalAwalTable(realPool);
        transaction = new sql.Transaction(realPool);
        await transaction.begin();
        transactionStarted = true;
        ...
        const existing = await pool.request()
            ...
            .query('SELECT stsflag FROM t_pengajuan WITH (UPDLOCK, HOLDLOCK) WHERE id_pengajuan = @id');
        ...
        const previousStsflag = existing.recordset[0].stsflag?.toString();
        if (previousStsflag !== PENGAJUAN_STATUS.APPROVAL_AWAL) {
            await transaction.rollback();
            transactionStarted = false;
            return res.status(400).json({
                status: 'error',
                success: false,
                message: `Pengajuan tidak berada di tahap Approval Awal (stsflag saat ini ${previousStsflag || '-'}).`,
            });
        }

        await updatePengajuanStsflag(pool, id, targetStsflag);
        await pool.request()
            .input('id_pengajuan', sql.VarChar, id)
            .input('action', sql.VarChar, action)
            .input('catatan', sql.NVarChar, catatan)
            .input('previous_stsflag', sql.VarChar, previousStsflag)
            .input('target_stsflag', sql.VarChar, targetStsflag)
            .input('userid', sql.VarChar, actor.userid)
            .input('username', sql.NVarChar, actor.username || null)
            .query(`
                INSERT INTO t_pengajuan_approval_awal
                    (id_pengajuan, action, catatan, previous_stsflag, target_stsflag, userid, username, created_at)
                VALUES
                    (@id_pengajuan, @action, @catatan, @previous_stsflag, @target_stsflag, @userid, @username, GETDATE())
            `);

        await transaction.commit();
        transactionStarted = false;

        const event = targetStsflag === PENGAJUAN_STATUS.KOREKSI_ADMIN
            ? 'koreksi'
            : targetStsflag === PENGAJUAN_STATUS.DITOLAK
                ? 'reject'
                : 'save';
        const whatsappNotification = queueWorkflowNotification(realPool, {
            idPengajuan: id,
            targetStsflag,
            event,
            previousStsflag,
        ...
        res.json({
            status: 'success',
            success: true,
            message: targetStsflag === PENGAJUAN_STATUS.FPK_AO
                ? 'Approval awal disetujui. Pengajuan lanjut ke FPK.'
                : targetStsflag === PENGAJUAN_STATUS.KOREKSI_ADMIN
                    ? 'Pengajuan dikoreksi ke Admin Kredit.'
                    : 'Pengajuan ditolak atau dibatalkan.',
            previous_stsflag: previousStsflag,
            stsflag: targetStsflag,
            whatsapp_notification: whatsappNotification,
        });
    } catch (error) {
        ...
        console.error('POST approval awal error:', error);
        res.status(500).json({
            status: 'error',
            success: false,
...
router.post('/api/pengajuan/verifikasi/:id', handleUpload, async (req, res) => {
    ...
    try {
        const realPool = await getPool();
        await maybeEnsureSchema(ensurePengajuanDetailTables, realPool);
        await maybeEnsureSchema(ensurePengajuanDukcapilTable, realPool);
        await maybeEnsureSchema(ensureVerifikasiDukcapilColumns, realPool);
        transaction = new sql.Transaction(realPool);
        await transaction.begin();
        transactionStarted = true;
        const pool = { request: () => new sql.Request(transaction) };
        const headerResult = await pool.request().input('id', sql.VarChar, id)
            .query('SELECT stsflag, screening_awal FROM t_pengajuan WITH (UPDLOCK, HOLDLOCK) WHERE id_pengajuan = @id');
        const header = headerResult.recordset[0];
        if (!header || !['1', '91'].includes(String(header.stsflag))) {
            await transaction.rollback(); transactionStarted = false;
            return res.status(409).json({ status: 'error', message: 'Pengajuan tidak berada pada tahap pemeriksaan admin.' });
        }
        const previousVerification = await pool.request().input('id', sql.VarChar, id)
            .query('SELECT TOP 1 file_hasil_dukcapil_debitur, file_hasil_dukcapil_pasangan FROM t_verifikasi_dukcapil WHERE id_pengajuan = @id ORDER BY verified_at DESC');
        const previous = previousVerification.recordset[0] || {};
        let screeningStatus;
        if (header.screening_awal) {
            const slikRows = await pool.request().input('id', sql.VarChar, id)
                .query('SELECT slik_data FROM t_pengajuan_slik WHERE id_pengajuan = @id');
            const hasSlik = slikRows.recordset.some(row => {
                try {
                    const data = typeof row.slik_data === 'string' ? JSON.parse(row.slik_data) : row.slik_data;
                    if (!data) return false;
                    const jenis = String(data.jenis || 'DEBITUR').toUpperCase();
                    if (!['DEBITUR', 'BADAN_USAHA'].includes(jenis)) return false;
                    const isNihil = data.is_nihil === true ||
                        String(data.status_slik || '').toUpperCase() === 'NIHIL' ||
                        String(data.kesimpulan || '').toUpperCase().includes('NIHIL');
                    const hasProof = Boolean(data.file_hasil_slik || data.file_slik || data.file_path || data.url);
                    return Boolean(data.nama_debitur || data.nama) || (isNihil && hasProof);
                } catch (_) { return false; }
            });
            const decision = screeningDecision({
                decision: body.keputusan_screening, note: catatanDebitur,
                hasSlik,
                hasDukcapil: Boolean(req.files?.some(f => f.fieldname === 'file_dukcapil_debitur') || previous.file_hasil_dukcapil_debitur),
                identityValid: status_debitur === true || status_debitur === 1,
            });
            if (decision.error) {
                await transaction.rollback(); transactionStarted = false;
                return res.status(400).json({ status: 'error', message: decision.error });
            }
        ...
        await pool.request()
            ...
            .input('file_hasil_dukcapil_debitur', sql.NVarChar, getFilePath('file_dukcapil_debitur') || previous.file_hasil_dukcapil_debitur || null)
            .input('file_hasil_dukcapil_pasangan', sql.NVarChar, getFilePath('file_dukcapil_pasangan') || previous.file_hasil_dukcapil_pasangan || null)
        ...
        const creditData = await pool.request()
            .input('id', sql.VarChar, id)
            .query('SELECT TOP 1 id_pengajuan FROM t_debitur_data_kredit WHERE id_pengajuan = @id');
        const nextStatus = screeningStatus || afterVerification(creditData.recordset.length > 0);

        // Data lengkap diteruskan ke pemeriksaan dokumen sebelum survei.
        await pool.request()
            .input('id', sql.VarChar, id)
            .input('stsflag', sql.VarChar, nextStatus)
        ...
        const whatsappNotification = queueWorkflowNotification(realPool, {
            idPengajuan: id,
            targetStsflag: nextStatus,
            event: 'save',
        });

        const verificationMessage = nextStatus === PENGAJUAN_STATUS.APPROVAL_AWAL
            ? 'Verifikasi selesai dan dikirim ke Supervisor Kredit untuk Approval Awal.'
            : nextStatus === PENGAJUAN_STATUS.KOREKSI_AO
                ? 'Pengajuan dikembalikan ke AO untuk perbaikan data.'
                : nextStatus === PENGAJUAN_STATUS.DITOLAK
                    ? 'Pengajuan ditandai tidak dilanjutkan.'
        ...
        if (transactionStarted) { try { await transaction.rollback(); } catch (_) {} }
...
// GET - List Pengajuan AO (draft/koreksi AO)
router.get('/api/pengajuan/list', async (req, res) => {
    try {
        const pool = await getPool();
        const request = pool.request();
        const statusParams = sqlInParams(request, 'stsflag', [
            PENGAJUAN_STATUS.DRAFT,
            PENGAJUAN_STATUS.KOREKSI_AO,
        ]);
        const result = await request.query(`
            SELECT a.id_pengajuan, a.jenis_debitur, a.plafon_pengajuan, a.status_pengajuan,
                   COALESCE(a.stsflag, '0') AS stsflag,
                   CASE WHEN a.jenis_debitur = 'PERORANGAN' THEN b.nama_debitur ELSE c.nama_perusahaan END AS nama
            FROM t_pengajuan a
            LEFT JOIN t_debitur_perorangan b ON a.id_pengajuan = CAST(b.id_pengajuan AS VARCHAR(50))
            LEFT JOIN t_debitur_badan_usaha c ON a.id_pengajuan = CAST(c.id_pengajuan AS VARCHAR(50))
            WHERE ISNULL(a.stsflag, ${PENGAJUAN_STATUS.DRAFT}) IN (${statusParams})
...
// GET - List FPK (stsflag = 2); koreksi AO (90) berada di Pengajuan Kredit.
router.get('/api/pengajuan/listFpk', async (req, res) => {
    const idAo = String(req.query?.id_ao || '').trim();
    if (!idAo) {
        return res.status(400).json({ message: 'ID AO wajib diisi untuk menampilkan pengajuan.' });
    }
    try {
        const pool = await getPool();
        const request = pool.request();
        request.input('id_ao', sql.VarChar(50), idAo);
        const statusParams = sqlInParams(request, 'stsflag', STATUS_LISTS.FPK);
        const result = await request.query(`
            SELECT a.id_pengajuan, a.jenis_debitur, a.plafon_pengajuan, a.status_pengajuan, a.screening_awal, a.persetujuan_slik,
                   CASE WHEN a.jenis_debitur = 'PERORANGAN' THEN b.nama_debitur ELSE c.nama_perusahaan END AS nama
            FROM t_pengajuan a
            LEFT JOIN t_debitur_perorangan b ON a.id_pengajuan = CAST(b.id_pengajuan AS VARCHAR(50))
            LEFT JOIN t_debitur_badan_usaha c ON a.id_pengajuan = CAST(c.id_pengajuan AS VARCHAR(50))
            WHERE a.stsflag IN (${statusParams})
              AND LTRIM(RTRIM(a.id_ao)) = @id_ao
            ORDER BY a.id_pengajuan DESC
        `);
        res.json(result.recordset);
    } catch (error) {
...
router.get('/api/pengajuan/listCheckKelengkapan', async (req, res) => {
    try {
        const pool = await getPool();
        const request = pool.request();
        const statusParams = sqlInParams(request, 'stsflag', STATUS_LISTS.CHECKLIST);
        const result = await request.query(`
            SELECT a.id_pengajuan, a.jenis_debitur, a.plafon_pengajuan, a.status_pengajuan,
                   CASE WHEN a.jenis_debitur = 'PERORANGAN' THEN b.nama_debitur ELSE c.nama_perusahaan END AS nama
            FROM t_pengajuan a
...
async function getPengajuanListByStsflags(pool, stsflags) {
    const values = Array.isArray(stsflags) ? stsflags : [stsflags];
    const request = pool.request();
    const statusParams = sqlInParams(request, 'stsflag', values);
...
function workflowListRoute(path, stsflags) {
    router.get(path, async (req, res) => {
        try {
            const pool = await getPool();
            const result = await getPengajuanListByStsflags(pool, stsflags);
            res.json(result.recordset);
        } catch (error) {
            res.status(500).json({ message: error.message });
        }
...
workflowListRoute('/api/pengajuan/listRekapAnalisa', STATUS_LISTS.REKAP_ANALISA);
workflowListRoute('/api/pengajuan/listSurveyDebitur', STATUS_LISTS.SURVEY_DEBITUR);
workflowListRoute('/api/pengajuan/listSurveyAgunan', STATUS_LISTS.SURVEY_AGUNAN);
workflowListRoute('/api/pengajuan/listMUK', STATUS_LISTS.MUK);

router.get('/api/pengajuan/listApprovalRekap', async (req, res) => {
    try {
        const pool = await getPool();
        ...
        const statusParams = sqlInParams(
            request,
            'stsflag',
            STATUS_LISTS.APPROVAL_REKAP
        );
        ...
        const result = await request.query(`
            SELECT
                ...
                r.biaya,
                r.sisa_penghasilan,
                r.sisa_penghasilan AS cash_flow,
                r.angsuran,
                r.dsr,
                r.created_at AS tgl_rekap,
                r.updated_at,

                CASE
                    WHEN r.dsr IS NULL THEN 'PERLU REVIEW'
                    WHEN r.dsr <= 35 THEN 'LAYAK'
                    WHEN r.dsr <= 50 THEN 'LAYAK DENGAN CATATAN'
                    ELSE 'PERLU REVIEW SUPERVISOR'
                END AS kesimpulan_ao,

                'MENUNGGU REVIEW' AS status_approval,

                CASE
                    WHEN a.jenis_debitur = 'PERORANGAN' THEN b.nama_debitur
                    ELSE c.nama_perusahaan
                END AS nama
            FROM t_pengajuan a
            LEFT JOIN t_debitur_perorangan b
                ON a.id_pengajuan = CAST(b.id_pengajuan AS VARCHAR(50))
            LEFT JOIN t_debitur_badan_usaha c
                ON a.id_pengajuan = CAST(c.id_pengajuan AS VARCHAR(50))
            LEFT JOIN t_pengajuan_rekap_analisa r
                ON a.id_pengajuan = CAST(r.id_pengajuan AS VARCHAR(50))
            WHERE a.stsflag IN (${statusParams})
              AND (
                    @search = '%%'
                    OR a.id_pengajuan LIKE @search
            ...
            ORDER BY ISNULL(r.updated_at, r.created_at) ASC, a.tgl_pengajuan ASC
        `);
        ...
        console.error('Error di listApprovalRekap:', error);
        res.status(500).json({
            status: 'error',
            success: false,
            message: error.message,
...
router.get('/api/pengajuan/listApprovalMuk', async (req, res) => {
    try {
        const pool = await getPool();
        const result = await getPengajuanListByStsflags(
            pool,
            PENGAJUAN_STATUS.MUK_REVIEW
        );

        const rows = await enrichRowsWithApprovalRoute(pool, result.recordset || []);
        res.json({
            success: true,
            data: rows,
        });
    } catch (error) {
        console.error('GET listApprovalMuk error:', error);
        res.status(500).json({
            success: false,
            message: error.message,
        });
...
router.get('/api/pengajuan/listApprovalDireksi', async (req, res) => {
    try {
        const pool = await getPool();
        ...
        const result = await request.query(`
            SELECT
                ...
                m.created_at AS tgl_muk,
                m.updated_at,
                ISNULL(m.updated_at, m.created_at) AS tgl_approval,

                'MENUNGGU REVIEW' AS status_direksi,
                'MENUNGGU REVIEW' AS status_approval,
            ...
            LEFT JOIN t_debitur_badan_usaha c
                ON a.id_pengajuan = CAST(c.id_pengajuan AS VARCHAR(50))
            LEFT JOIN t_pengajuan_muk m
                ON a.id_pengajuan = CAST(m.id_pengajuan AS VARCHAR(50))
            WHERE LTRIM(RTRIM(CAST(a.stsflag AS VARCHAR(10)))) IN (${statusParams})
              AND (
                    @search = '%%'
                    OR a.id_pengajuan LIKE @search
                    OR ISNULL(b.nama_debitur, '') LIKE @search
              ...
              AND (
                    @status = ''
                    OR @status = 'SEMUA'
                    OR (@status = 'MENUNGGU' AND 'MENUNGGU REVIEW' LIKE '%MENUNGGU%')
              )
            ORDER BY ISNULL(m.updated_at, m.created_at) ASC, a.tgl_pengajuan ASC
        `);
        ...
        console.error('GET listApprovalDireksi error:', error);
        return res.status(500).json({
            status: 'error',
            success: false,
            message: error.message,
...
// V10.2 - Endpoint unik agar tidak bentrok dengan /api/pengajuan/:id.
router.get('/api/approval-matrix/config', async (req, res) => {
    try {
        const pool = await getPool();
        const policy = await getApprovalPolicy(pool);
        const rules = await getApprovalRules(pool);
        return res.json({
            success: true,
            data: {
                version_no: Number(policy.version_no || 1),
                komisaris_min_plafon: Number(policy.komisaris_min_plafon || 0),
                updated_by: policy.updated_by || null,
                updated_at: policy.updated_at || null,
                rules: rules.map((r) => ({
                    ...r,
                    min_plafon: r.min_plafon == null ? null : Number(r.min_plafon),
                    max_plafon: r.max_plafon == null ? null : Number(r.max_plafon),
                    require_manager: toBooleanFlag(r.require_manager),
                    require_direksi: toBooleanFlag(r.require_direksi),
                    require_komisaris: toBooleanFlag(r.require_komisaris),
                    aktif: toBooleanFlag(r.aktif),
                })),
            },
        });
    } catch (error) {
        console.error('GET approval matrix config error:', error);
        return res.status(500).json({ success: false, message: error.message });
    }
});

router.put('/api/approval-matrix/config', async (req, res) => {
    try {
        const pool = await getPool();
        await ensureApprovalMatrixTables(pool);
        const actor = await canManageApprovalPolicy(pool, req);
        if (!actor) {
            return res.status(403).json({
                success: false,
                message: 'Matriks Kewenangan Kredit hanya dapat diubah oleh Administrator, Super User, atau Direksi.',
            });
        }

        const threshold = Number(req.body?.komisaris_min_plafon);
        const reason = req.body?.alasan_perubahan?.toString().trim() || '';
        const incomingRules = Array.isArray(req.body?.rules) ? req.body.rules : [];
        if (!Number.isFinite(threshold) || threshold < 0) {
            return res.status(400).json({ success: false, message: 'Batas plafon kebijakan tidak valid.' });
        }
        if (!reason) {
            return res.status(400).json({ success: false, message: 'Alasan perubahan wajib diisi untuk audit trail.' });
        }
        if (!incomingRules.length) {
            return res.status(400).json({ success: false, message: 'Minimal harus ada satu rule matriks.' });
        }

        const allowedDebtor = new Set(['SEMUA', 'PERORANGAN_UMUM', 'KARYAWAN_INTERNAL', 'BADAN_USAHA']);
        const allowedSource = new Set(['SEMUA', 'GAJI', 'HASIL_USAHA', 'GAJI_USAHA', 'PENDAPATAN_LAIN', 'LAINNYA']);
        const allowedPlafond = new Set(['SEMUA', 'DI_BAWAH_BATAS', 'MINIMAL_BATAS', 'CUSTOM']);
        const rules = incomingRules.map((raw, index) => {
            const rule = raw && typeof raw === 'object' ? raw : {};
            const nama = rule.nama_rule?.toString().trim() || `Rule ${index + 1}`;
            const kategori = rule.kategori_debitur?.toString().trim().toUpperCase() || 'SEMUA';
            const sumber = rule.sumber_pengembalian?.toString().trim().toUpperCase() || 'SEMUA';
            const mode = rule.plafon_mode?.toString().trim().toUpperCase() || 'SEMUA';
            const manager = toBooleanFlag(rule.require_manager);
            const direksi = toBooleanFlag(rule.require_direksi);
            const komisaris = toBooleanFlag(rule.require_komisaris);
            ...
            if (!allowedDebtor.has(kategori)) throw new Error(`Kategori debitur rule "${nama}" tidak valid.`);
            if (!allowedSource.has(sumber)) throw new Error(`Sumber pengembalian rule "${nama}" tidak valid.`);
            if (!allowedPlafond.has(mode)) throw new Error(`Kondisi plafon rule "${nama}" tidak valid.`);
            if (!manager && !direksi && !komisaris) throw new Error(`Rule "${nama}" harus memiliki minimal satu level approval.`);
            if (mode === 'CUSTOM') {
                if (min != null && (!Number.isFinite(min) || min < 0)) throw new Error(`Minimum plafon rule "${nama}" tidak valid.`);
                if (max != null && (!Number.isFinite(max) || max < 0)) throw new Error(`Maksimum plafon rule "${nama}" tidak valid.`);
                if (min != null && max != null && min > max) throw new Error(`Rentang plafon rule "${nama}" tidak valid.`);
            }
            return {
                nama_rule: nama,
                priority: Number.isFinite(Number(rule.priority)) ? Math.max(1, Math.round(Number(rule.priority))) : (index + 1) * 10,
                kategori_debitur: kategori,
                sumber_pengembalian: sumber,
                plafon_mode: mode,
                min_plafon: mode === 'CUSTOM' ? min : null,
                max_plafon: mode === 'CUSTOM' ? max : null,
                require_manager: manager,
                require_direksi: direksi,
                require_komisaris: komisaris,
                aktif: rule.aktif === undefined ? true : toBooleanFlag(rule.aktif),
            };
        });

        const current = await getApprovalPolicy(pool);
        const nextVersion = Number(current.version_no || 1) + 1;
        const transaction = new sql.Transaction(pool);
        await transaction.begin();
        try {
            await new sql.Request(transaction)
                .input('threshold', sql.Decimal(18, 2), threshold)
                .input('version_no', sql.Int, nextVersion)
                .input('userid', sql.VarChar, actor.userid)
                .query(`
                    UPDATE dbo.t_approval_policy
                    SET komisaris_min_plafon = @threshold,
                        internal_employee_requires_komisaris = 0,
                        version_no = @version_no,
                        updated_by = @userid,
                        updated_at = GETDATE()
                    WHERE id_policy = 1;
                    DELETE FROM dbo.t_approval_policy_rule;
                `);

            for (const rule of rules) {
                await new sql.Request(transaction)
                    .input('version_no', sql.Int, nextVersion)
                    .input('nama_rule', sql.NVarChar, rule.nama_rule)
                    .input('priority', sql.Int, rule.priority)
                    .input('kategori_debitur', sql.VarChar, rule.kategori_debitur)
                    .input('sumber_pengembalian', sql.VarChar, rule.sumber_pengembalian)
                    .input('plafon_mode', sql.VarChar, rule.plafon_mode)
                    .input('min_plafon', sql.Decimal(18, 2), rule.min_plafon)
                    .input('max_plafon', sql.Decimal(18, 2), rule.max_plafon)
                    .input('require_manager', sql.Bit, rule.require_manager)
                    .input('require_direksi', sql.Bit, rule.require_direksi)
                    .input('require_komisaris', sql.Bit, rule.require_komisaris)
                    .input('aktif', sql.Bit, rule.aktif)
                    .query(`
                        INSERT INTO dbo.t_approval_policy_rule
                            (version_no, nama_rule, priority, kategori_debitur, sumber_pengembalian,
                             plafon_mode, min_plafon, max_plafon, require_manager,
                             require_direksi, require_komisaris, aktif, created_at, updated_at)
                        VALUES
                            (@version_no, @nama_rule, @priority, @kategori_debitur, @sumber_pengembalian,
                             @plafon_mode, @min_plafon, @max_plafon, @require_manager,
                             @require_direksi, @require_komisaris, @aktif, GETDATE(), GETDATE())
                    `);
            }
            ...
            await new sql.Request(transaction)
                .input('version_no', sql.Int, nextVersion)
                .input('threshold', sql.Decimal(18, 2), threshold)
                .input('rules_json', sql.NVarChar, JSON.stringify(rules))
                .input('reason', sql.NVarChar, reason)
                .input('userid', sql.VarChar, actor.userid)
                .input('username', sql.NVarChar, actor.username || null)
                .query(`
                    INSERT INTO dbo.t_approval_policy_history
                        (version_no, komisaris_min_plafon, internal_employee_requires_komisaris,
                         rules_json, alasan_perubahan, changed_by, changed_by_name, changed_at)
                    VALUES
                        (@version_no, @threshold, 0, @rules_json, @reason, @userid, @username, GETDATE())
                `);
            await transaction.commit();
        } catch (error) {
            await transaction.rollback();
        ...
        return res.json({
            success: true,
            message: `Matriks Kewenangan Kredit berhasil disimpan sebagai versi ${nextVersion}.`,
            data: { version_no: nextVersion, komisaris_min_plafon: threshold, rules },
        });
    } catch (error) {
        console.error('PUT approval matrix config error:', error);
        return res.status(400).json({ success: false, message: error.message });
    }
});

router.get('/api/approval-matrix/history', async (req, res) => {
    try {
        const pool = await getPool();
        await ensureApprovalMatrixTables(pool);
        const result = await pool.request().query(`
            SELECT TOP 50 id_history, version_no, komisaris_min_plafon,
                   rules_json, alasan_perubahan, changed_by, changed_by_name, changed_at
            FROM dbo.t_approval_policy_history
            ORDER BY changed_at DESC, id_history DESC
        `);
...
router.get('/api/approval-matrix/route/:id', async (req, res) => {
    try {
        const pool = await getPool();
        const route = await getApprovalRoute(pool, req.params.id);
        if (!route) return res.status(404).json({ success: false, message: 'Pengajuan tidak ditemukan.' });
...
// V10 - Pengaturan matriks kewenangan kredit.
router.get('/api/pengajuan/approval-policy', async (req, res) => {
    try {
        const pool = await getPool();
        const policy = await getApprovalPolicy(pool);
        return res.json({
            ...
            data: {
                ...policy,
                komisaris_min_plafon: Number(policy.komisaris_min_plafon || 0),
                internal_employee_requires_komisaris: toBooleanFlag(policy.internal_employee_requires_komisaris),
            },
        });
    } catch (error) {
...
router.put('/api/pengajuan/approval-policy', async (req, res) => {
    try {
        const pool = await getPool();
        await ensureApprovalMatrixTables(pool);
        const actor = await canManageApprovalPolicy(pool, req);
        if (!actor) {
            return res.status(403).json({
                success: false,
                message: 'Pengaturan Matriks Kewenangan Kredit hanya dapat diubah oleh Administrator, Super User, atau Direksi.',
            });
        }

        const threshold = Number(req.body?.komisaris_min_plafon);
        const internalRequired = toBooleanFlag(req.body?.internal_employee_requires_komisaris);
        const reason = req.body?.alasan_perubahan?.toString().trim() || '';

        if (!Number.isFinite(threshold) || threshold < 0) {
            return res.status(400).json({ success: false, message: 'Batas plafon Komisaris tidak valid.' });
        }
        if (!reason) {
            return res.status(400).json({ success: false, message: 'Alasan perubahan kebijakan wajib diisi untuk audit trail.' });
        }
        ...
        try {
            await new sql.Request(transaction)
                .input('version_no', sql.Int, nextVersion)
                .input('komisaris_min_plafon', sql.Decimal(18, 2), threshold)
                .input('internal_required', sql.Bit, internalRequired)
                .input('userid', sql.VarChar, actor.userid)
                .query(`
                    UPDATE dbo.t_approval_policy
                    SET komisaris_min_plafon = @komisaris_min_plafon,
                        internal_employee_requires_komisaris = @internal_required,
                        version_no = @version_no,
                        updated_by = @userid,
                        updated_at = GETDATE()
                    WHERE id_policy = 1
            ...
            await new sql.Request(transaction)
                ...
                .input('komisaris_min_plafon', sql.Decimal(18, 2), threshold)
                .input('internal_required', sql.Bit, internalRequired)
                .input('alasan', sql.NVarChar, reason)
                .input('userid', sql.VarChar, actor.userid)
                .input('username', sql.NVarChar, actor.username || null)
                .query(`
                    INSERT INTO dbo.t_approval_policy_history
                        (version_no, komisaris_min_plafon, internal_employee_requires_komisaris,
                         alasan_perubahan, changed_by, changed_by_name, changed_at)
                    VALUES
                        (@version_no, @komisaris_min_plafon, @internal_required,
                         @alasan, @userid, @username, GETDATE())
                `);
            await transaction.commit();
        } catch (error) {
            await transaction.rollback();
        ...
        const updated = await getApprovalPolicy(pool);
        return res.json({
            success: true,
            message: 'Matriks Kewenangan Kredit berhasil diperbarui. Pengajuan yang sudah memiliki snapshot tetap memakai kebijakan saat snapshot dibuat.',
            data: {
                ...updated,
                komisaris_min_plafon: Number(updated.komisaris_min_plafon || 0),
                internal_employee_requires_komisaris: toBooleanFlag(updated.internal_employee_requires_komisaris),
            },
        });
    } catch (error) {
        console.error('PUT approval policy error:', error);
        return res.status(500).json({ success: false, message: error.message });
    }
});

router.get('/api/pengajuan/approval-policy/history', async (req, res) => {
    try {
        const pool = await getPool();
        await ensureApprovalMatrixTables(pool);
        const result = await pool.request().query(`
            SELECT TOP 50 id_history, version_no, komisaris_min_plafon,
                   internal_employee_requires_komisaris, alasan_perubahan,
...
router.get('/api/pengajuan/:id/approval-route', async (req, res) => {
    try {
        const pool = await getPool();
        const route = await getApprovalRoute(pool, req.params.id);
        if (!route) return res.status(404).json({ success: false, message: 'Pengajuan tidak ditemukan.' });
...
router.get('/api/pengajuan/listApprovalKomisaris', async (req, res) => {
    try {
        const pool = await getPool();
        await ensureApprovalMatrixTables(pool);
        const search = req.query.search?.toString().trim() || '';
        const request = pool.request().input('search', sql.NVarChar, `%${search}%`);
        const statusParams = sqlInParams(request, 'stsflag', STATUS_LISTS.APPROVAL_KOMISARIS);
        const result = await request.query(`
            SELECT a.id_pengajuan, a.jenis_debitur, a.plafon_pengajuan, a.tenor_bulan,
                   a.tgl_pengajuan, a.status_pengajuan, a.catatan_ao, a.stsflag, a.id_ao,
                   b.nama_debitur, b.no_ktp, b.no_hp,
                   c.nama_perusahaan, c.npwp,
                   r.snapshot_at AS tgl_approval,
                   'MENUNGGU PERSETUJUAN KOMISARIS' AS status_approval,
                   CASE WHEN a.jenis_debitur = 'PERORANGAN' THEN b.nama_debitur ELSE c.nama_perusahaan END AS nama
            FROM dbo.t_pengajuan a
            LEFT JOIN dbo.t_debitur_perorangan b ON a.id_pengajuan = CAST(b.id_pengajuan AS VARCHAR(50))
            LEFT JOIN dbo.t_debitur_badan_usaha c ON a.id_pengajuan = CAST(c.id_pengajuan AS VARCHAR(50))
            LEFT JOIN dbo.t_pengajuan_approval_route r ON a.id_pengajuan = r.id_pengajuan
            WHERE a.stsflag IN (${statusParams})
              AND (
                    @search = '%%'
                    OR a.id_pengajuan LIKE @search
            ...
            ORDER BY ISNULL(r.snapshot_at, a.tgl_pengajuan) ASC
        `);
        const rows = await enrichRowsWithApprovalRoute(pool, result.recordset || []);
        return res.json({ success: true, data: rows });
    } catch (error) {
        console.error('GET listApprovalKomisaris error:', error);
        return res.status(500).json({ success: false, message: error.message });
    }
});
...
router.get('/api/pengajuan/:id/muk', async (req, res) => {
    ...
    try {
        const pool = await getPool();
        await maybeEnsureSchema(ensureMukTable, pool);
        await maybeEnsureSchema(ensureRekapAnalisaTable, pool);
        console.time('SELECT_DETAIL');
        const result = await pool.request()
            .input('id_pengajuan', sql.VarChar, id)
            .query(`
...
router.post('/api/pengajuan/:id/mutasi/upload', handleSingleMutasiUpload, async (req, res) => {
    ...
    try {
        ...
        const pool = await getPool();
        await maybeEnsureSchema(ensureMutasiRekeningTables, pool);
...
router.post('/api/pengajuan/:id/muk', async (req, res) => {
    ...
    try {
        ...
        const isDraft = isDraftSubmission(req.body);
        let whatsappNotification = null;
        let approvalRoute = null;
        let targetStsflag = PENGAJUAN_STATUS.MUK;
        if (!isDraft) {
            const snapshotBy = req.body?.userid?.toString().trim() || req.get('x-userid')?.toString().trim() || null;
            // V10.2: setiap MUK dikirim ulang setelah koreksi, route dievaluasi ulang
            // dan disimpan sebagai revision baru. History revision sebelumnya tetap ada.
            approvalRoute = await saveApprovalRouteSnapshot(pool, id, snapshotBy, { forceRefresh: true });
            targetStsflag = approvalStatusFromRoute(approvalRoute.route);
            await updatePengajuanStsflag(pool, id, targetStsflag);
            whatsappNotification = queueWorkflowNotification(pool, {
                idPengajuan: id,
                targetStsflag,
                event: 'muk',
            });
        ...
        const firstRole = approvalRoute ? firstApprovalRole(approvalRoute.route) : null;
        res.json({
            status: 'success',
            message: isDraft
                ? 'Draft MUK berhasil disimpan'
                : firstRole
                    ? `MUK dikirim ke ${approvalRoleLabel(firstRole)} sesuai Matriks Kewenangan Kredit.`
                    : 'MUK selesai tanpa tahap approval tambahan.',
            is_draft: isDraft,
            stsflag: targetStsflag,
...
router.get('/api/pengajuan/:id/kelengkapan', async (req, res) => {
    ...
    try {
        const pool = await getPool();
        await maybeEnsureSchema(ensureKelengkapanDokumenTable, pool);
...
router.post('/api/pengajuan/:id/kelengkapan', handleUpload, async (req, res) => {
    ...
    const isDraft = body.is_draft === true || body.save_as_draft === true || body.mode === 'draft';
    ...
    try {
        ...
        let whatsappNotification = null;
        if (!isDraft) {
            await updatePengajuanStsflag(pool, id, WORKFLOW_NEXT_STATUS.CHECKLIST_LANJUT);
            whatsappNotification = queueWorkflowNotification(pool, {
                idPengajuan: id,
                targetStsflag: WORKFLOW_NEXT_STATUS.CHECKLIST_LANJUT,
                event: 'save',
            });
        }
        ...
        res.json({
            status: 'success',
            message: isDraft ? 'Draft kelengkapan berhasil disimpan.' : 'Kelengkapan berhasil disimpan.',
            is_draft: isDraft,
            stsflag: isDraft ? undefined : WORKFLOW_NEXT_STATUS.CHECKLIST_LANJUT,
            whatsapp_notification: whatsappNotification,
        });
    } catch (error) {
...
router.post('/api/pengajuan/:id/progress', async (req, res) => {
    ...
    if (!ALLOWED_PROGRESS_STATUSES.has(normalizedStsflag)) {
        return res.status(400).json({
            status: 'error',
...
// AO decides after admin screening; ownership and stage checked under lock.
router.post('/api/pengajuan/:id/keputusan-ao', async (req, res) => {
    const { decision, reason, userid } = req.body || {};
    if (!['lanjut', 'batal'].includes(decision) || !String(userid || '').trim() ||
        (decision === 'batal' && !String(reason || '').trim())) {
        return res.status(400).json({ message: 'Keputusan, pengguna, dan alasan pembatalan wajib diisi.' });
    }
    let transaction;
    let active = false;
    try {
        const pool = await getPool();
        transaction = new sql.Transaction(pool);
        await transaction.begin(); active = true;
        const result = await new sql.Request(transaction).input('id', sql.VarChar, req.params.id)
            .query('SELECT stsflag, id_ao, screening_awal, persetujuan_slik FROM t_pengajuan WITH (UPDLOCK, HOLDLOCK) WHERE id_pengajuan = @id');
        const row = result.recordset[0];
        if (!row || String(row.id_ao || '').trim() !== String(userid).trim() || !row.screening_awal || String(row.stsflag) !== '2') {
            await transaction.rollback(); active = false;
            return res.status(409).json({ message: 'Pengajuan tidak berada pada tahap tindak lanjut AO atau bukan milik Anda.' });
        }
        const consent = { ...readConsent(row.persetujuan_slik), ao_decision: decision,
            ao_reason: String(reason || '').trim(), ao_decided_by: userid, ao_decided_at: new Date().toISOString() };
        await new sql.Request(transaction)
            .input('id', sql.VarChar, req.params.id)
            .input('consent', sql.NVarChar(sql.MAX), JSON.stringify(consent))
            .input('stage', sql.VarChar, decision === 'batal' ? '99' : '2')
            .query('UPDATE t_pengajuan SET persetujuan_slik = @consent, stsflag = @stage WHERE id_pengajuan = @id');
        await transaction.commit(); active = false;
        res.json({ status: 'success', message: decision === 'batal' ? 'Pengajuan dibatalkan.' : 'Silakan lengkapi data pengajuan.' });
    } catch (error) {
        await rollbackIfActive(active, transaction);
        res.status(500).json({ message: error.message });
    }
});
...
router.post('/api/pengajuan/:id/koreksi', async (req, res) => {
    console.time('LOAD_PENGAJUAN');
    const { id } = req.params;
    const { target_stsflag, catatan_koreksi } = req.body;
    const catatanKoreksi = catatan_koreksi?.toString().trim();

    if (!ALLOWED_KOREKSI_STATUSES.has(String(target_stsflag))) {
        console.timeEnd('LOAD_PENGAJUAN');
        return res.status(400).json({
            status: 'error',
...
async function ensureSurveyDebiturInfoTable(pool) {
    await ensureTableColumns(pool, 't_pengajuan_survey_debitur_info', [
        ...
        { name: 'status_pegawai_debitur', type: 'NVARCHAR(50)' },
        { name: 'status_pegawai_pasangan', type: 'NVARCHAR(50)' },
        { name: 'created_at', type: 'DATETIME' },
        { name: 'updated_at', type: 'DATETIME' },
    ]);
}
...
router.get('/api/pengajuan/:id/survey-debitur', async (req, res) => {
    try {
        const pool = await getPool();
        await maybeEnsureSchema(ensureSurveyDebiturNarasumberTable, pool);
        await maybeEnsureSchema(ensureSurveyDebiturInfoTable, pool);
        ...
        const infoResult = await pool.request()
            ...
            .query(`
                SELECT TOP 1
                    ...
                    status_pegawai_debitur,
                    status_pegawai_pasangan,
                    status_tempat_kerja,
                    kondisi_bangunan,
                    lama_tinggal,
                    mengikuti_ormas,
        ...
        for (const row of narasumberResult.recordset) {
            ...
            } else if (jenis === 'KARYAWAN_DEBITUR') {
                response.karyawan_debitur.push(item);
            } else if (jenis === 'KARYAWAN_PASANGAN') {
                response.karyawan_pasangan.push(item);
            } else {
...
router.post('/api/pengajuan/:id/survey-debitur', handleUpload, async (req, res) => {
    ...
    const karyawanDebitur = Array.isArray(data.karyawan_debitur) ? data.karyawan_debitur : [];
    const karyawanPasangan = Array.isArray(data.karyawan_pasangan) ? data.karyawan_pasangan : [];
    ...
    const surveyInfo = {
        ...
        status_pegawai_debitur: data.status_pegawai_debitur || null,
        status_pegawai_pasangan: data.status_pegawai_pasangan || null,
    ...
    try {
        ...
        try {
            ...
            await new sql.Request(transaction)
                ...
                .input('status_pegawai_debitur', sql.NVarChar, surveyInfo.status_pegawai_debitur)
                .input('status_pegawai_pasangan', sql.NVarChar, surveyInfo.status_pegawai_pasangan)
        ...
        BEGIN
            ...
            SET tanggal_survey = @tanggal_survey,
                ...
                status_pegawai_debitur = @status_pegawai_debitur,
                status_pegawai_pasangan = @status_pegawai_pasangan,
    ...
    `);
            ...
            await insertRows('KARYAWAN_DEBITUR', karyawanDebitur);
            await insertRows('KARYAWAN_PASANGAN', karyawanPasangan);
            ...
            await updatePengajuanStsflag(pool, idPengajuan, WORKFLOW_NEXT_STATUS.SURVEY_DEBITUR_SIMPAN);
            const whatsappNotification = queueWorkflowNotification(pool, {
                idPengajuan,
                targetStsflag: WORKFLOW_NEXT_STATUS.SURVEY_DEBITUR_SIMPAN,
                event: 'save',
            });
            res.json({
                status: 'success',
...
router.get('/api/pengajuan/:id/survey-agunan', async (req, res) => {
    try {
        const pool = await getPool();
        await maybeEnsureSchema(ensureSurveyAgunanTable, pool);
...
router.post('/api/pengajuan/:id/survey-agunan', handleUpload, async (req, res) => {
    ...
    try {
        ...
        try {
            ...
            await transaction.commit();
            await updatePengajuanStsflag(pool, idPengajuan, WORKFLOW_NEXT_STATUS.SURVEY_AGUNAN_SIMPAN);
            const whatsappNotification = queueWorkflowNotification(pool, {
                idPengajuan,
                targetStsflag: WORKFLOW_NEXT_STATUS.SURVEY_AGUNAN_SIMPAN,
                event: 'save',
            });
            res.json({
                status: 'success',
    ...
    try {
        ...
        if (!isDraft) {
            ...
            const survey = await pool.request()
                .input('id_pengajuan', sql.VarChar, idPengajuan)
                .query('SELECT TOP 1 id_pengajuan FROM t_pengajuan_survey_agunan WHERE id_pengajuan = @id_pengajuan');
            const nextStatus = afterAnalisa(survey.recordset.length > 0);
            await updatePengajuanStsflag(pool, idPengajuan, nextStatus);
            whatsappNotification = queueWorkflowNotification(pool, {
                idPengajuan,
                targetStsflag: nextStatus,
                event: 'save',
        ...
        res.status(200).json({
            status: 'success',
            message: isDraft ? 'Draft analisa berhasil disimpan' : 'Analisa tersimpan. Lanjutkan sesuai tahap pengajuan.',
            is_draft: isDraft,
            whatsapp_notification: whatsappNotification,
        });
    } catch (error) {
...
router.get('/api/pengajuan/:id', async (req, res) => {
    ...
    try {
        ...
        const result = await pool.request()
            ...
            .query(`
                SELECT a.id_pengajuan, a.jenis_debitur, a.plafon_pengajuan, a.tenor_bulan, a.tgl_pengajuan, 
                       a.status_pengajuan, a.catatan_ao, a.stsflag, a.screening_awal, a.persetujuan_slik,
                       ISNULL(ac.is_karyawan_internal, 0) AS is_karyawan_internal,
                       ac.nip_karyawan_internal, ac.unit_kerja_internal, ac.jabatan_karyawan_internal,
                       b.nama_debitur, b.no_ktp, b.no_hp, 
                       ...
                       k.sumber_pembayaran_kredit, k.kategori_sumber_pengembalian, k.detail_tujuan, k.sindikasi, k.alasan,
                       k.asuransi_jiwa, k.asuransi_kredit, k.asuransi_lainnya
                FROM t_pengajuan a
                LEFT JOIN t_pengajuan_approval_context ac ON a.id_pengajuan = ac.id_pengajuan
                LEFT JOIN t_debitur_perorangan b ON a.id_pengajuan = CAST(b.id_pengajuan AS VARCHAR(50))
                LEFT JOIN t_debitur_badan_usaha c ON a.id_pengajuan = CAST(c.id_pengajuan AS VARCHAR(50))
        ...
        await Promise.all([
            maybeEnsureSchema(ensureVerifikasiDukcapilColumns, pool),
            maybeEnsureSchema(ensurePengajuanDukcapilTable, pool),
        ]);
        ...
        const [
            ...
            pool.request()
                ...
                .query(`
                    ...
                    agama, nama_ibu_kandung, pekerjaan, alamat_ktp, alamat_domisili, domisili_sesuai_ktp
                    FROM t_pengajuan_penjamin 
        ...
        for (const s of slikData.recordset) {
            try {
                ...
                listSlik.push({
                    ...
                    file_hasil_slik: parsedData.file_hasil_slik || parsedData.file_slik || null,
                    file_slik: parsedData.file_slik || parsedData.file_hasil_slik || null,
                    file_path: parsedData.file_path || parsedData.path || parsedData.url || null,
                    url: parsedData.url || parsedData.file_path || parsedData.path || null,
                    lampiran: parsedData.lampiran || parsedData.dokumen || null,
                });
...
router.delete('/api/pengajuan/:id', async (req, res) => {
    ...
    try {
        ...
        const summary = await getPengajuanNotificationSummary(pool, id, {
            id_pengajuan: id,
        });

        await deletePengajuanDatabaseRows(pool, id);

        res.json({
            success: true,
            message: 'Data pengajuan berhasil dihapus. File diproses di background.',
        });

        cleanupPengajuanStorage(pool, id)
            .then((deletedFiles) => {
                console.log(`[DELETE ${id}] cleanup Backblaze selesai: ${deletedFiles} file`);
            })
            .catch((err) => {
                console.error(`[DELETE ${id}] cleanup Backblaze gagal:`, err.message);
            });

        notifyWorkflowUsersSafe(pool, {
            idPengajuan: id,
            fallback: summary,
            targetOverride: deleteNotificationTarget,
            event: 'delete',
        }).catch((err) => {
            console.error(`[DELETE ${id}] WA gagal:`, err.message);
        });
    } catch (error) {
...
router.post('/api/pengajuan/upload-ttd', upload.single('ttd_file'), async (req, res) => {
    try {
        ...
        if (tipe_ttd === 'debitur') {
            ...
            await maybeEnsureSchema((db) => ensureColumn(db, 't_debitur_perorangan', 'ttd_debitur', 'NVARCHAR(500)'), pool);
            await pool.request()
                .input('id_pengajuan', sql.VarChar, id_pengajuan)
            ...
            await maybeEnsureSchema((db) => ensureColumn(db, 't_debitur_perorangan', 'ttd_pasangan', 'NVARCHAR(500)'), pool);
            await pool.request()
                .input('id_pengajuan', sql.VarChar, id_pengajuan)
                .input('ttd_url', sql.NVarChar, ttdUrl)
            ...
            await maybeEnsureSchema((db) => ensureColumn(db, 't_pengajuan_penjamin', 'ttd_penjamin', 'NVARCHAR(500)'), pool);
            ...
            await maybeEnsureSchema((db) => ensureColumn(db, 't_pengajuan_pendiri', 'ttd_pendiri', 'NVARCHAR(500)'), pool);
...
async function ensurePengajuanDatabase(pool = null) {
    const db = pool || await getPool();
    const steps = [
        ['muser.pass', ensureMuserPasswordColumn],
        ['muser.foto_profile', ensureMuserProfileColumn],
        ['muser.theme_mode', ensureMuserThemeModeColumn],
        ['dokumen', ensureDokumenTable],
        ['pengajuan detail', ensurePengajuanDetailTables],
        ['dukcapil pengajuan', ensurePengajuanDukcapilTable],
        ['dukcapil verifikasi', ensureVerifikasiDukcapilColumns],
        ['muk', ensureMukTable],
        ['rekap analisa', ensureRekapAnalisaTable],
        ['mutasi rekening', ensureMutasiRekeningTables],
        ['kelengkapan dokumen', ensureKelengkapanDokumenTable],
        ['survey debitur narasumber', ensureSurveyDebiturNarasumberTable],
        ['survey debitur info', ensureSurveyDebiturInfoTable],
        ['survey agunan', ensureSurveyAgunanTable],
        ['pengajuan log', ensurePengajuanLogTable],
        ['approval matrix', ensureApprovalMatrixTables],
    ];
...
router.get('/api/nasabah/proses', async (req, res) => {
    try {
      ...
      SELECT
        ...
        CASE 
          WHEN p.stsflag = 0 THEN 'Draft / Baru dibuat AO'
          WHEN p.stsflag = 1 THEN 'Menunggu Verifikasi Admin Kredit'
          WHEN p.stsflag = 10 THEN 'Menunggu Approval Awal Supervisor'
          WHEN p.stsflag = 2 THEN 'FPK Pengajuan oleh AO'
          WHEN p.stsflag = 3 THEN 'Checklist Kelengkapan Admin Kredit'
          WHEN p.stsflag = 4 THEN 'Rekap dan Analisa Kredit'
          WHEN p.stsflag = 5 THEN 'Survey Debitur'
          WHEN p.stsflag = 6 THEN 'Survey Agunan'
          WHEN p.stsflag = 7 THEN 'Review MUK'
          WHEN p.stsflag = 8 THEN 'Penyusunan MUK'
          WHEN p.stsflag = 9 THEN 'Putusan Kredit Direksi'
          WHEN p.stsflag = 11 THEN 'Persetujuan Komisaris'
          WHEN p.stsflag = 41 THEN 'Approval Rekap (Berkas Lama)'
          WHEN p.stsflag = 90 THEN 'Koreksi ke AO'
          WHEN p.stsflag = 91 THEN 'Koreksi ke Admin Kredit'
          WHEN p.stsflag = 99 THEN 'Ditolak / Dibatalkan'
      ...
      WHERE ISNULL(p.stsflag, 0) IN (0, 1, 10, 11, 2, 3, 4, 41, 5, 6, 7, 8, 9, 90, 91)
...
router.ensureDatabase = ensurePengajuanDatabase;
router.initializeDatabase = ensurePengajuanDatabase;
router.runDbEnsureOnRequest = runDbEnsureOnRequest;

router.put('/api/users/:userid/theme-mode', async (req, res) => {
    try {
        const { userid } = req.params;
        const { theme_mode } = req.body;

        const requestedMode = String(theme_mode || '').toLowerCase();
        const mode = ['light', 'dark', 'clay'].includes(requestedMode)
            ? requestedMode
            : 'light';
        ...
        await pool.request()
            .input('userid', sql.VarChar(30), userid)
            .input('theme_mode', sql.VarChar(20), mode)
            .query(`
                UPDATE muser
...
// =====================================================
// APPROVAL AWAL - SUPERVISOR
// =====================================================

async function ensureApprovalAwalTable(pool) {
    await pool.request().query(`
    IF OBJECT_ID('dbo.t_pengajuan_approval_awal', 'U') IS NULL
    BEGIN
      CREATE TABLE dbo.t_pengajuan_approval_awal (
        id_approval INT IDENTITY(1,1) PRIMARY KEY,
        id_pengajuan VARCHAR(50) NOT NULL,
        action VARCHAR(20) NOT NULL,
        catatan NVARCHAR(MAX) NULL,
        skor_kelengkapan INT NULL,
        skor_slik INT NULL,
        skor_dukcapil INT NULL,
        skor_risiko INT NULL,
        previous_stsflag VARCHAR(10) NULL,
        target_stsflag VARCHAR(10) NULL,
        userid VARCHAR(50) NULL,
    ...
    END
  `);
    await ensureTableColumns(pool, 't_pengajuan_approval_awal', [
        { name: 'action', type: 'VARCHAR(20)' },
        { name: 'catatan', type: 'NVARCHAR(MAX)' },
        { name: 'previous_stsflag', type: 'VARCHAR(10)' },
        { name: 'target_stsflag', type: 'VARCHAR(10)' },
        { name: 'userid', type: 'VARCHAR(50)' },
        { name: 'username', type: 'NVARCHAR(100)' },
        { name: 'created_at', type: 'DATETIME' },
    ]);
...
async function ensureApprovalRekapTable(pool) {
    await pool.request().query(`
        IF OBJECT_ID('dbo.t_pengajuan_approval_rekap', 'U') IS NULL
        BEGIN
            CREATE TABLE dbo.t_pengajuan_approval_rekap (
                id_approval INT IDENTITY(1,1) PRIMARY KEY,
                id_pengajuan VARCHAR(50) NOT NULL,
                action VARCHAR(20) NOT NULL,
                catatan NVARCHAR(MAX) NULL,
        ...
        END
    `);
    await ensureTableColumns(pool, 't_pengajuan_approval_rekap', [
        { name: 'action', type: 'VARCHAR(20)' },
        { name: 'catatan', type: 'NVARCHAR(MAX)' },
        { name: 'previous_stsflag', type: 'VARCHAR(10)' },
        { name: 'target_stsflag', type: 'VARCHAR(10)' },
...
router.post('/api/pengajuan/:id/approval-rekap', async (req, res) => {
    const { id } = req.params;
    const action = (req.body?.action || '').toString().toLowerCase().trim();
    const catatan = req.body?.catatan?.toString().trim() || null;
    const userid = req.body?.userid?.toString().trim() || req.get('x-userid') || null;
    ...
    const targetByAction = {
        approve: WORKFLOW_NEXT_STATUS.APPROVAL_REKAP_SETUJUI,
        setujui: WORKFLOW_NEXT_STATUS.APPROVAL_REKAP_SETUJUI,
        koreksi: WORKFLOW_NEXT_STATUS.APPROVAL_REKAP_KOREKSI,
        tolak: WORKFLOW_NEXT_STATUS.APPROVAL_REKAP_TOLAK,
        reject: WORKFLOW_NEXT_STATUS.APPROVAL_REKAP_TOLAK,
    };
    ...
    if (!targetStsflag) {
        return res.status(400).json({
            success: false,
            message: 'Aksi approval rekap tidak valid.',
        });
    }
    ...
    try {
        const pool = await getPool();
        const actor = await requireWorkflowJabatan(pool, req, res, ['14', '15'], 'Approval Rekap');
        if (!actor) return;
        await ensureApprovalRekapTable(pool);
        ...
        if (previousStsflag !== PENGAJUAN_STATUS.APPROVAL_REKAP) {
            ...
            if (
                ['approve', 'setujui'].includes(action) &&
                alreadyApprovedStatuses.has(previousStsflag)
            ) {
                return res.json({
                    success: true,
                    message: 'Pengajuan sudah melewati Approval Rekap. Daftar akan diperbarui.',
                    previous_stsflag: previousStsflag,
                    stsflag: previousStsflag,
            ...
            return res.status(400).json({
                success: false,
                message: `Pengajuan tidak berada di tahap Approval Rekap. Status saat ini ${previousStsflag || '-'}.`,
            });
        }
        ...
        await pool.request()
            ...
            .input('username', sql.NVarChar, actor.username || null)
            .query(`
                INSERT INTO t_pengajuan_approval_rekap
                    (id_pengajuan, action, catatan, previous_stsflag, target_stsflag, userid, username, created_at)
                VALUES
                    (@id_pengajuan, @action, @catatan, @previous_stsflag, @target_stsflag, @userid, @username, GETDATE())
            `);

        const event = targetStsflag === PENGAJUAN_STATUS.REKAP_ANALISA
            ? 'koreksi'
            : targetStsflag === PENGAJUAN_STATUS.DITOLAK
                ? 'reject'
                : 'save';
        ...
        res.json({
            success: true,
            message: targetStsflag === PENGAJUAN_STATUS.SURVEY_DEBITUR
                ? 'Approval rekap disetujui. Pengajuan lanjut ke Survey Debitur.'
                : targetStsflag === PENGAJUAN_STATUS.REKAP_ANALISA
                    ? 'Pengajuan dikoreksi ke Rekap dan Analisa.'
                    : 'Pengajuan ditolak.',
            previous_stsflag: previousStsflag,
            stsflag: targetStsflag,
            whatsapp_notification: whatsappNotification,
        ...
        console.error('POST approval rekap error:', error);
        res.status(500).json({
            success: false,
            message: error.message,
        });
...
async function ensureApprovalMukTable(pool) {
    await pool.request().query(`
        IF OBJECT_ID('dbo.t_pengajuan_approval_muk', 'U') IS NULL
        BEGIN
            CREATE TABLE dbo.t_pengajuan_approval_muk (
                id_approval INT IDENTITY(1,1) PRIMARY KEY,
                id_pengajuan VARCHAR(50) NOT NULL,
                action VARCHAR(20) NOT NULL,
                catatan NVARCHAR(MAX) NULL,
    ...
    await ensureTableColumns(pool, 't_pengajuan_approval_muk', [
        { name: 'action', type: 'VARCHAR(20)' },
        { name: 'catatan', type: 'NVARCHAR(MAX)' },
        { name: 'previous_stsflag', type: 'VARCHAR(10)' },
        { name: 'target_stsflag', type: 'VARCHAR(10)' },
...
router.post('/api/pengajuan/:id/approval-muk', async (req, res) => {
    const { id } = req.params;
    const action = (req.body?.action || '').toString().toLowerCase().trim();
    const catatan = req.body?.catatan?.toString().trim() || null;

    const targetByAction = {
        approve: WORKFLOW_NEXT_STATUS.APPROVAL_MUK_SETUJUI,
        setujui: WORKFLOW_NEXT_STATUS.APPROVAL_MUK_SETUJUI,
        koreksi: WORKFLOW_NEXT_STATUS.APPROVAL_MUK_KOREKSI,
        tolak: WORKFLOW_NEXT_STATUS.APPROVAL_MUK_TOLAK,
        reject: WORKFLOW_NEXT_STATUS.APPROVAL_MUK_TOLAK,
    };

    let targetStsflag = targetByAction[action];
    if (!targetStsflag) {
        return res.status(400).json({ success: false, message: 'Aksi approval MUK tidak valid.' });
    }
    if (['koreksi', 'tolak', 'reject'].includes(action) && !catatan) {
        return res.status(400).json({ success: false, message: 'Catatan wajib diisi.' });
    }

    try {
        const pool = await getPool();
        const actor = await requireWorkflowJabatan(pool, req, res, ['15'], 'Review MUK');
        if (!actor) return;
        await ensureApprovalMukTable(pool);
        await ensureApprovalMatrixTables(pool);

        const existing = await pool.request()
            .input('id_pengajuan', sql.VarChar, id)
            .query(`SELECT TOP 1 stsflag FROM t_pengajuan WHERE id_pengajuan = @id_pengajuan`);
        ...
        const previousStsflag = existing.recordset[0].stsflag?.toString();
        if (previousStsflag !== PENGAJUAN_STATUS.MUK_REVIEW) {
            return res.status(409).json({
                success: false,
                message: `Pengajuan tidak berada di tahap Review MUK. Status saat ini ${previousStsflag || '-'}.`,
            });
        }

        let approvalRoute = null;
        if (action === 'approve' || action === 'setujui') {
            await ensureMukTable(pool);
            const muk = await pool.request()
                .input('id_pengajuan', sql.VarChar, id)
                .query('SELECT TOP 1 id_pengajuan FROM t_pengajuan_muk WHERE id_pengajuan = @id_pengajuan');
            if (!muk.recordset.length) {
                targetStsflag = PENGAJUAN_STATUS.MUK;
            } else {
                approvalRoute = await getApprovalRoute(pool, id);
                if (!approvalRoute?.snapshot) {
                    approvalRoute = await saveApprovalRouteSnapshot(pool, id, actor.userid);
                }
                if (!approvalRoute.route.includes('MANAGER')) {
                    return res.status(409).json({
                        success: false,
                        message: 'Snapshot matriks pengajuan ini tidak memiliki tahap Manager.',
                    });
                }
                targetStsflag = approvalStatusFromRoute(approvalRoute.route, 'MANAGER');
            }
        }
        ...
        await pool.request()
            ...
            .input('username', sql.NVarChar, actor.username || null)
            .query(`
                INSERT INTO t_pengajuan_approval_muk
                    (id_pengajuan, action, catatan, previous_stsflag, target_stsflag, userid, username, created_at)
                VALUES
                    (@id_pengajuan, @action, @catatan, @previous_stsflag, @target_stsflag, @userid, @username, GETDATE())
            `);
        ...
        queueWorkflowNotification(pool, {
            idPengajuan: id,
            targetStsflag,
            event: targetStsflag === PENGAJUAN_STATUS.MUK ? 'koreksi' : 'save',
            previousStsflag,
            catatan,
        });
        ...
        return res.json({
            success: true,
            message: ['approve', 'setujui'].includes(action)
                ? targetStsflag === PENGAJUAN_STATUS.DISETUJUI
                    ? 'Review MUK disetujui Manager. Sesuai matriks, approval selesai dan pengajuan dapat dilanjutkan ke Dokumen Kredit.'
                    : `Review MUK disetujui Manager. Pengajuan lanjut ke ${approvalRoleLabel(nextApprovalRole(approvalRoute?.route || [], 'MANAGER'))}.`
                : targetStsflag === PENGAJUAN_STATUS.MUK
                    ? 'Pengajuan dikembalikan ke penyusunan/perbaikan MUK.'
                    : 'Pengajuan ditolak.',
            previous_stsflag: previousStsflag,
            stsflag: targetStsflag,
            approval_route: approvalRoute,
        ...
        console.error('POST approval MUK error:', error);
        return res.status(500).json({ success: false, message: error.message });
    }
});


async function ensureApprovalDireksiTable(pool) {
    await pool.request().query(`
        IF OBJECT_ID('dbo.t_pengajuan_approval_direksi', 'U') IS NULL
        BEGIN
            CREATE TABLE dbo.t_pengajuan_approval_direksi (
                id_approval INT IDENTITY(1,1) PRIMARY KEY,
                id_pengajuan VARCHAR(50) NOT NULL,
                action VARCHAR(20) NOT NULL,
                catatan NVARCHAR(MAX) NULL,
    ...
    await ensureTableColumns(pool, 't_pengajuan_approval_direksi', [
        { name: 'action', type: 'VARCHAR(20)' },
        { name: 'catatan', type: 'NVARCHAR(MAX)' },
        { name: 'previous_stsflag', type: 'VARCHAR(10)' },
        { name: 'target_stsflag', type: 'VARCHAR(10)' },
...
router.post('/api/pengajuan/:id/approval-direksi', async (req, res) => {
    const { id } = req.params;
    const action = (req.body?.action || '').toString().toLowerCase().trim();
    const catatan = req.body?.catatan?.toString().trim() || null;

    if (!['approve', 'setujui', 'koreksi', 'tolak', 'reject'].includes(action)) {
        return res.status(400).json({ success: false, message: 'Aksi approval Direksi tidak valid.' });
    }
    if (['koreksi', 'tolak', 'reject'].includes(action) && !catatan) {
        return res.status(400).json({ success: false, message: 'Catatan wajib diisi.' });
    }
    ...
    try {
        const pool = await getPool();
        const actor = await requireWorkflowJabatan(pool, req, res, ['17'], 'Putusan Kredit Direksi');
        if (!actor) return;
        await ensureApprovalDireksiTable(pool);
        await ensureApprovalMatrixTables(pool);
        ...
        const previousStsflag = existing.recordset[0].stsflag?.toString();
        if (previousStsflag !== PENGAJUAN_STATUS.APPROVAL_FINAL) {
            return res.status(409).json({
                success: false,
                message: `Pengajuan tidak berada di Putusan Kredit Direksi. Status saat ini: ${previousStsflag || '-'}.`,
            });
        }

        let approvalRoute = await getApprovalRoute(pool, id);
        if (!approvalRoute?.snapshot) {
            approvalRoute = await saveApprovalRouteSnapshot(pool, id, actor.userid);
        }

        if (!approvalRoute.route.includes('DIREKSI')) {
            return res.status(409).json({
                success: false,
                message: 'Snapshot matriks pengajuan ini tidak memiliki tahap Direksi.',
            });
        }

        let targetStsflag;
        if (['approve', 'setujui'].includes(action)) {
            targetStsflag = approvalStatusFromRoute(approvalRoute.route, 'DIREKSI');
        ...
        await pool.request()
            ...
            .input('username', sql.NVarChar, actor.username || null)
            .query(`
                INSERT INTO t_pengajuan_approval_direksi
                    (id_pengajuan, action, catatan, previous_stsflag, target_stsflag, userid, username, created_at)
                VALUES
                    (@id_pengajuan, @action, @catatan, @previous_stsflag, @target_stsflag, @userid, @username, GETDATE())
            `);
        ...
        const nextRoleAfterDireksi = nextApprovalRole(approvalRoute.route, 'DIREKSI');
        const approvedMessage = nextRoleAfterDireksi
            ? `Putusan Direksi disetujui. Pengajuan lanjut ke ${approvalRoleLabel(nextRoleAfterDireksi)} sesuai matriks (${approvalRoute.matched_rule_name || approvalRoute.alasan_route}).`
            : 'Putusan Direksi disetujui. Sesuai matriks, approval selesai dan pengajuan dapat dilanjutkan ke Dokumen Kredit.';
        ...
        return res.json({
            ...
            message: ['approve', 'setujui'].includes(action)
                ? approvedMessage
                : targetStsflag === PENGAJUAN_STATUS.MUK
                    ? 'Pengajuan dikoreksi kembali ke MUK. Review Manager dan Putusan Direksi harus dilakukan ulang.'
                    : 'Pengajuan ditolak.',
            previous_stsflag: previousStsflag,
            stsflag: targetStsflag,
            approval_route: approvalRoute,
        ...
        console.error('POST approval direksi error:', error);
        return res.status(500).json({ success: false, message: error.message });
    }
});

router.post('/api/pengajuan/:id/approval-komisaris', async (req, res) => {
    const { id } = req.params;
    const action = (req.body?.action || '').toString().toLowerCase().trim();
    const catatan = req.body?.catatan?.toString().trim() || null;

    if (!['approve', 'setujui', 'koreksi', 'tolak', 'reject'].includes(action)) {
        return res.status(400).json({ success: false, message: 'Aksi Persetujuan Komisaris tidak valid.' });
    }
    if (['koreksi', 'tolak', 'reject'].includes(action) && !catatan) {
        return res.status(400).json({ success: false, message: 'Catatan wajib diisi.' });
    }
    ...
    try {
        const pool = await getPool();
        const actor = await requireWorkflowJabatan(pool, req, res, ['18'], 'Persetujuan Komisaris');
        if (!actor) return;
        await ensureApprovalMatrixTables(pool);
        ...
        const previousStsflag = existing.recordset[0].stsflag?.toString();
        if (previousStsflag !== PENGAJUAN_STATUS.APPROVAL_KOMISARIS) {
            return res.status(409).json({
                success: false,
                message: `Pengajuan tidak berada di Persetujuan Komisaris. Status saat ini: ${previousStsflag || '-'}.`,
            });
        }

        const approvalRoute = await getApprovalRoute(pool, id);
        if (!approvalRoute?.route?.includes('KOMISARIS')) {
            return res.status(409).json({
                success: false,
                message: 'Snapshot matriks pengajuan ini tidak memiliki tahap Komisaris.',
            });
        }

        const targetStsflag = ['approve', 'setujui'].includes(action)
            ? approvalStatusFromRoute(approvalRoute.route, 'KOMISARIS')
            : action === 'koreksi'
                ? PENGAJUAN_STATUS.MUK
        ...
        await pool.request()
            ...
            .input('username', sql.NVarChar, actor.username || null)
            .query(`
                INSERT INTO dbo.t_pengajuan_approval_komisaris
                    (id_pengajuan, action, catatan, previous_stsflag, target_stsflag, userid, username, created_at)
                VALUES
                    (@id_pengajuan, @action, @catatan, @previous_stsflag, @target_stsflag, @userid, @username, GETDATE())
            `);
        ...
        return res.json({
            success: true,
            message: ['approve', 'setujui'].includes(action)
                ? 'Persetujuan Komisaris selesai. Pengajuan disetujui dan dapat dilanjutkan ke Dokumen Kredit.'
                : targetStsflag === PENGAJUAN_STATUS.MUK
                    ? 'Komisaris meminta koreksi. Pengajuan dikembalikan ke MUK dan jalur approval harus dijalankan ulang.'
                    : 'Pengajuan ditolak oleh Komisaris.',
            previous_stsflag: previousStsflag,
            stsflag: targetStsflag,
            approval_route: approvalRoute,
        });
    } catch (error) {
        console.error('POST approval komisaris error:', error);
        return res.status(500).json({ success: false, message: error.message });
    }
});

module.exports = router;
router.getUserMenuAccess = getUserMenuAccess;
router.getAccessTables = getAccessTables;
