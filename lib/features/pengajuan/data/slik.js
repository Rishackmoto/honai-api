...
// =======================================================
// SLIK NIHIL / TIDAK DITEMUKAN - bukti PDF/JPG/PNG
// =======================================================
const uploadSlikNihil = multer({
    ...
    fileFilter: (req, file, callback) => {
        ...
        if (file.fieldname !== 'proof_file') {
            return callback(new Error(`Field file tidak dikenali: ${file.fieldname}`));
        }
        if (!['.pdf', '.jpg', '.jpeg', '.png'].includes(extension)) {
            return callback(new Error('Bukti SLIK nihil harus PDF/JPG/JPEG/PNG.'));
        }
        callback(null, true);
    },
});

async function existingSlikRowsForPerson(pool, id, normalizedJenis, normalizedIndex) {
    const result = await pool.request()
    ...
    for (const row of result.recordset || []) {
        ...
        const oldJenis = String(parsed.jenis || 'DEBITUR').toUpperCase();
        const oldIndex = parsed.index ?? parsed.index_ke ?? null;
        if (oldJenis === normalizedJenis && String(oldIndex ?? '') === String(normalizedIndex ?? '')) {
            matched.push({ row, parsed });
        }
    }
    return matched;
}
...
router.post(
    '/api/pengajuan/slik/:id/nihil',
    uploadSlikNihil.single('proof_file'),
    async (req, res) => {
        let savedFile = null;
        try {
            const pool = await getPool();
            const id = req.params.id;
            const normalizedJenis = String(req.body?.jenis || 'DEBITUR').toUpperCase().trim();
            const normalizedIndex = req.body?.index === undefined || req.body?.index === ''
                ? null
                : Number(req.body.index);
            const userid = String(req.body?.userid || req.get('x-userid') || '').trim() || null;
            const catatan = String(req.body?.catatan || '').trim() || null;
            const nama = String(req.body?.nama || '').trim() || null;
            const nik = String(req.body?.nik || '').trim() || null;

            if (!req.file) {
                return res.status(400).json({
                    success: false,
                    message: 'Bukti SLIK nihil wajib diupload (PDF/JPG/JPEG/PNG).',
                });
            }

            const allowedJenis = new Set(['DEBITUR', 'BADAN_USAHA', 'PASANGAN', 'PENJAMIN', 'PENDIRI', 'PENGURUS']);
            if (!allowedJenis.has(normalizedJenis)) {
                return res.status(400).json({ success: false, message: 'Jenis pemilik SLIK tidak valid.' });
            }

            const filename = `${Date.now()}-slik-nihil-${safeFilename(req.file.originalname)}`;
            const key = `pengajuan/${id}/slik_nihil/${filename}`;
            const uploadedPath = await uploadToB2({
                key,
                buffer: req.file.buffer,
                contentType: req.file.mimetype,
            });
            savedFile = { path: uploadedPath, key };

            const oldRows = await existingSlikRowsForPerson(pool, id, normalizedJenis, normalizedIndex);
            const oldProofs = oldRows
                .map(({ parsed }) => parsed.file_hasil_slik || parsed.file_slik || null)
                .filter(Boolean);
            ...
            const finalSlikData = {
                ...
                status_slik: 'NIHIL',
                is_nihil: true,
                kesimpulan: 'SLIK NIHIL / Tidak Ditemukan',
                file_hasil_slik: uploadedPath,
                file_slik: uploadedPath,
                bukti_nama: req.file.originalname,
                bukti_mime: req.file.mimetype,
            ...
            try {
                if (oldRows.length) {
                    const ids = oldRows.map(({ row }) => row.id_slik).filter(Boolean);
                    for (const idSlik of ids) {
                        await new sql.Request(transaction)
                            .input('id_slik', sql.Int, idSlik)
                            .query('DELETE FROM t_pengajuan_slik WHERE id_slik = @id_slik');
                    }
                }
            ...
            if (oldProofs.length) {
                deleteManyFromB2(oldProofs).catch((error) =>
                    console.warn('Cleanup bukti SLIK lama gagal:', error.message)
                );
            }
            ...
            res.json({
                success: true,
                status: 'success',
                message: 'SLIK nihil berhasil disimpan beserta bukti.',
                data: finalSlikData,
            });
        } catch (error) {
            if (savedFile) {
                await cleanupSavedFiles([savedFile]).catch(() => {});
            }
            console.error('SAVE SLIK NIHIL ERROR:', error);
            res.status(500).json({
                success: false,
                status: 'error',
                message: error.message || 'Gagal menyimpan SLIK nihil',
            });
        }
    }
);
    ...
    try {
        ...
        for (const row of existing.recordset || []) {
            ...
            if (
                ...
                const oldProof = parsed.file_hasil_slik || parsed.file_slik || null;
                if (oldProof) oldProofsToDelete.push(oldProof);
            }
        }

        if (idsToDelete.length > 0) {
            for (const idSlik of idsToDelete) {
                await pool.request()
                    .input('id_slik', sql.Int, idSlik)
                    .query('DELETE FROM t_pengajuan_slik WHERE id_slik = @id_slik');
            }
        ...
        if (oldProofsToDelete.length > 0) {
            deleteManyFromB2(oldProofsToDelete).catch((error) =>
                console.warn('Cleanup bukti SLIK nihil lama gagal:', error.message)
            );
        }
