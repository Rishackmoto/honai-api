...
const { createWorkflowInAppNotifications } = require('./in_app_notification');

const { PENGAJUAN_STATUS } = require('./workflow');
...
const workflowNotificationTargets = {
    ...
    [PENGAJUAN_STATUS.MUK_REVIEW]: {
        jabatanCodes: ['15'],
        menu: 'Review dan Putusan - Review MUK',
        action: 'melakukan review MUK',
    },
    [PENGAJUAN_STATUS.MUK]: {
        jabatanCodes: ['15', '17'],
        menu: 'MUK',
        action: 'menyusun atau memperbaiki MUK',
    },
    [PENGAJUAN_STATUS.APPROVAL_FINAL]: {
        jabatanCodes: ['17'],
        menu: 'Review dan Putusan - Putusan Kredit',
        action: 'memberikan putusan kredit',
    },
    [PENGAJUAN_STATUS.APPROVAL_KOMISARIS]: {
        jabatanCodes: ['18'],
        menu: 'Review dan Putusan - Persetujuan Komisaris',
        action: 'memberikan persetujuan Komisaris',
    },
    ...
    [PENGAJUAN_STATUS.DISETUJUI]: {
        ...
        targetAo: true,
        menu: 'Dokumen Kredit',
        action: 'menindaklanjuti kredit yang telah disetujui',
    },
...
async function getPengajuanNotificationSummary(pool, idPengajuan, fallback = {}) {
    ...
    const result = await pool.request()
        ...
        .query(`
            ...
            LEFT JOIN muser ao ON a.id_ao = ao.userid
...
async function getActiveNotificationUsers(pool, target = {}, summary = {}) {
    ...
    const aoBranch = summary.ao_kdcab?.toString().trim() || '';
    ...
    if (codes.length) {
        if (aoBranch && codes.some((code) => ['13', '14', '15'].includes(code))) {
            request.input('ao_kdcab', sql.VarChar, aoBranch);
        }
        for (const code of codes) {
            if (aoBranch && ['13', '14', '15'].includes(code)) {
                conditions.push(`(jabat = '${code}' AND ISNULL(kdcab, '') = @ao_kdcab)`);
            } else {
                conditions.push(`jabat = '${code}'`);
            }
        }
    }
    if (aoUserid) {
...
async function notifyWorkflowUsers(pool, {
    ...
    let inAppNotification = { inserted: 0 };
    try {
        inAppNotification = await createWorkflowInAppNotifications(pool, {
            users: recipients,
            summary,
            target,
            targetStsflag,
        ...
        inAppNotification = { inserted: 0, failed: true, error: error.message };
    }

    const whatsappRecipients = recipients.filter((user) => {
        const phone = user.nohp?.toString().replace(/\D/g, '');
        return Boolean(phone);
    });

    if (!whatsappRecipients.length) {
        console.log(`[WA] skipped ${event} ${summary.id_pengajuan || '-'} -> ${target.menu}: no phone recipients`);
        return {
            skipped: true,
            reason: 'Tidak ada user penerima WA yang aktif/bernomor HP',
            in_app_notification: inAppNotification,
        };
    }

    console.log(`[WA] sending ${event} ${summary.id_pengajuan || '-'} -> ${target.menu} to ${whatsappRecipients.length} user(s): ${whatsappRecipients.map((user) => user.userid).join(', ')}`);
    ...
    const results = [];
    for (const user of whatsappRecipients) {
        try {
