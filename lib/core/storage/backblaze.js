...
const allowedPrivatePrefixes = ["pengajuan/", "profile/"];
...
function keyFromB2Url(value) {
  ...
  try {
    ...
    if (nestedKey) {
      return keyFromB2Url(nestedKey);
    }

    if (parts[0] === "file" && parts[1] === bucketName) {
      return normalizeB2Key(parts.slice(2).join("/"));
    }
    if (parts[0] === bucketName) {
      return normalizeB2Key(parts.slice(1).join("/"));
    }
